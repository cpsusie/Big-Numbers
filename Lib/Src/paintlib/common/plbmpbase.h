...
public:
  ...
  PLPixel32 GetPixel32(int x, int y) const;

  //! Function to get a 24 bpp pixel.
  PLPixel24 GetPixel24(int x, int y) const;
  
  //! Function to get a 16 bpp pixel.
  PLPixel16 GetPixel16(int x, int y) const;
...
inline void PLBmpBase::SetPaletteEntry(PLBYTE Entry,PLBYTE r,PLBYTE g,PLBYTE b,PLBYTE a) {
  m_pClrTab[Entry].Set (r, g, b, a);
}

inline void PLBmpBase::SetPaletteEntry(PLBYTE Entry,PLPixel32 Value) {
...
// PLBmpBase direct manipulation
inline PLBYTE **PLBmpBase::GetLineArray() const {
  PLASSERT (m_pLineArray);
  return m_pLineArray;
}

inline PLPixel32 **PLBmpBase::GetLineArray32() const {
  PLASSERT (m_pLineArray);
  PLASSERT (GetBitsPerPixel() == 32);
  return (PLPixel32**)m_pLineArray;
}

inline PLPixel24 **PLBmpBase::GetLineArray24() const {
  PLASSERT (m_pLineArray);
  PLASSERT (GetBitsPerPixel() == 24);
  return (PLPixel24**)m_pLineArray;
}

inline PLPixel16 **PLBmpBase::GetLineArray16() const {
  PLASSERT (m_pLineArray);
  PLASSERT (GetBitsPerPixel() == 16);
  return (PLPixel16**)m_pLineArray;
}

inline PLPixel32 *PLBmpBase::GetPalette() const {
