...
/--------------------------------------------------------------------
|
|      $Id: plbmpdec.cpp,v 1.14 2004/09/11 12:41:35 uzadow Exp $
...
// Creates a decoder
PLBmpDecoder::PLBmpDecoder() : m_pBMI(0) {
}
...
void PLBmpDecoder::Open(PLDataSource *pDataSrc) {
  ...
  SetBmpInfo (PLPoint (m_pBMI->biWidth, m_pBMI->biHeight), DPI, pf);
}
...
void PLBmpDecoder::GetImage(PLBmpBase &Bmp) {
  ...
  switch (m_pBMI->biBitCount) {
    ...
    default:
      ...
      raiseError (PL_ERRFORMAT_UNKNOWN, "Decoding bmp: Illegal bpp value.");
  }
}
...
WINBITMAPINFOHEADER * PLBmpDecoder::getInfoHeader(PLDataSource *pDataSrc, PLPixel32 *pPal) {
  ...
  if (!((BFH.bfType == 0x4142) || (BFH.bfType == 0x4d42) ||
    ...
    raiseError (PL_ERRWRONG_SIGNATURE,"Bitmap decoder: This isn't a bitmap.");
...
void PLBmpDecoder::decode1bpp(PLDataSource *pDataSrc, PLBmpBase *pBmp) {
  int      XSize       = pBmp->GetWidth(); // Width of bitmap in pixels.
  int      LineLen     = ((XSize+7)/8 + 3) & ~3; // Width of source in bytes (DWORD-aligned).
  int      LinePadding = LineLen-((XSize+7)/8);
  PLBYTE **pLineArray  = pBmp->GetLineArray(); // Pointers to dest. lines.
  ...
  *(((PLBYTE*)&OpaqueBlack)+PL_RGBA_ALPHA) = 0xFF;
  ...
  for (int i=0; i<8; i++) { // Initialize bit masks.
    BTable[i] = 1<<i;
  }

  for (int y = 0; y < pBmp->GetHeight(); y++) { // For each line...
    PLBYTE *pDest = pLineArray[pBmp->GetHeight()-y-1];
    for (int x = 0; x < XSize/8; x++) { // For each source byte...
      PLBYTE *pSrc = pDataSrc->Read1Byte();
      PLBYTE  SrcByte = *(pSrc);
      for(int i = 7; i >= 0; i--,pDest++) { // For each bit...
    ...
    if(XSize & 7) {
      PLBYTE *pSrc = pDataSrc->Read1Byte();
      PLBYTE SrcByte = *(pSrc);
      for (int i=7; i>7-(XSize & 7); i--,pDest++) { // For each bit...
        if (SrcByte & BTable[i]) // Test if bit i is set
          *pDest = 0x01;
         else
          *pDest = 0x00;
...
void PLBmpDecoder::decode4bpp( PLDataSource * pDataSrc, PLBmpBase * pBmp) {
  int      XSize       = pBmp->GetWidth();       // Width of bitmap in pixels.
  int      LineLen     = ((XSize+1)/2 + 3) & ~3; // Width of source in bytes (DWORD-aligned).
  ...
  for (int y = 0; y < pBmp->GetHeight(); y++) { // For each line...
    PLBYTE *pDest = pLineArray[pBmp->GetHeight()-y-1];
    for (int x = 0; x < XSize/2; x++) { // For each source byte...
      PLBYTE *pSrc = pDataSrc->Read1Byte();
      PLBYTE SrcByte = *(pSrc);
      *pDest = SrcByte>>4;
      pDest++;
...
void PLBmpDecoder::decode8bpp(PLDataSource *pDataSrc, PLBmpBase *pBmp) {
  int    XSize       = pBmp->GetWidth(); // Width of bitmap in pixels.
  int    LineLen     = (XSize + 3) & ~3; // Width of source in bytes (DWORD-aligned).
  ...
  for(int y = 0; y < pBmp->GetHeight(); y++) { // For each line...
    PLBYTE *pDest = pLineArray[pBmp->GetHeight()-y-1];
    for (int x = 0; x < XSize; x++) { // For each source byte...
      PLBYTE *pSrc = pDataSrc->Read1Byte();
      *pDest = *pSrc;
      pDest++;
    }
...
void PLBmpDecoder::decodeRLE4(PLDataSource *pDataSrc, PLBmpBase *pBmp) {
  int XSize = pBmp->GetWidth();  // Width of bitmap in pixels.
  PLBYTE *pSrc;
  ...
  // Current line as uncompressed nibbles.
  // Allocate enough memory for DWORD alignment in original 4 bpp bitmap.
  PLBYTE *pLineBuf = new PLBYTE [XSize*4+28];

  for (int y = 0; y < pBmp->GetHeight() && !bEOF; y++) { // For each line...
    PLBYTE *pBuf = pLineBuf;
    bool bEOL=false;
    while(!bEOL) { // For each packet do
      pSrc = pDataSrc->Read1Byte();
      PLBYTE RunLength = *pSrc;
      if(RunLength==0) { // Literal or escape.
        pSrc = pDataSrc->Read1Byte();
        RunLength = *pSrc;
        switch(RunLength) {
          ...
          case 2: // Delta escape.
            ...
            delete [] pLineBuf;
            raiseError (PL_ERRFORMAT_NOT_SUPPORTED, "Encountered delta escape.");
            break;
          default:
            // Literal packet
            bOdd = (RunLength & 1);
            RunLength /= 2; // Convert pixels to bytes.
            for (int i=0; i<RunLength; i++) { // For each source byte...
              pSrc = pDataSrc->Read1Byte();
              decode2Nibbles (pBuf, *pSrc);
              pBuf += 2;
            }
            if (bOdd) { // Odd length packet -> one nibble left over
              pSrc = pDataSrc->Read1Byte();
              *pBuf = (*(pSrc))>>4;
              pBuf++;
            }
...
void PLBmpDecoder::decodeRLE8(PLDataSource *pDataSrc, PLBmpBase *pBmp) {
...
void PLBmpDecoder::decodeHiColor(PLDataSource *pDataSrc, PLBmpBase *pBmp, WINBITMAPINFOHEADER *pBMI) {
...
void PLBmpDecoder::decodeTrueColor(PLDataSource *pDataSrc, PLBmpBase *pBmp, int SrcBPP) {
  int y;                            // Current row
  PLBYTE * pDest;                   // Current destination.
  PLBYTE ** pLineArray = pBmp->GetLineArray();
                                   // Pointers to dest. lines.
  ...
  for (y=0; y<pBmp->GetHeight(); y++) { // For each line...
    pDest = pLineArray[pBmp->GetHeight()-y-1];
    switch (SrcBPP) {
      case 15:
...
void PLBmpDecoder::decode15bppLine(PLDataSource *pDataSrc, PLBmpBase *pBmp, PLBYTE *pDest) {
  int LineLen = (pBmp->GetWidth()*2 + 3) & ~3; // Width of source in bytes
                                               // (DWORD-aligned).
  int LinePadding = LineLen - pBmp->GetWidth()*2;

  for (int x = 0; x < pBmp->GetWidth(); x++) { // For each pixel...
...
void PLBmpDecoder::decode16bppLine(PLDataSource *pDataSrc,PLBmpBase * pBmp, PLBYTE * pDest) {
  int LineLen = (pBmp->GetWidth()*2 + 3) & ~3; // Width of source in bytes
                                               // (DWORD-aligned).
  int LinePadding = LineLen - pBmp->GetWidth()*2;
...
void PLBmpDecoder::decode24bppLine(PLDataSource *pDataSrc, PLBmpBase *pBmp, PLBYTE *pDest) {
  int LineLen = (pBmp->GetWidth()*3 + 3) & ~3; // Width of source in bytes
                                               // (DWORD-aligned).
  int LinePadding = LineLen - pBmp->GetWidth()*3;

  for (int x=0; x<pBmp->GetWidth(); x++) { // For each pixel...
    PLBYTE *pSrc = pDataSrc->ReadNBytes(3);
...
void PLBmpDecoder::decode32bppLine( PLDataSource *pDataSrc, PLBmpBase *pBmp, PLBYTE *pDest) {
  PLBYTE *pSrc = pDataSrc->ReadNBytes (pBmp->GetWidth()*4);
  memcpy (pDest, pSrc, pBmp->GetWidth()*4);
}

// Decodes two 4-bit pixels.
void PLBmpDecoder::decode2Nibbles(PLBYTE *pDest, PLBYTE SrcByte) {
  *pDest = SrcByte>>4;
  *(pDest+1) = SrcByte & 15;
}

// Assumes 8 bpp or less.
int PLBmpDecoder::readPalette(WINBITMAPINFOHEADER *pBMI, PLDataSource *pDataSrc, PLPixel32 *pPal, int RGBSize) {
  ...
  for (int i=0; i<NumColors; i++) {
    ...
    pPal[i].Set (pCurEntry->rgbRed, pCurEntry->rgbGreen, pCurEntry->rgbBlue, 0xFF);
  }
