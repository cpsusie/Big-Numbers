...
GLOBAL(void) jpeg_mem_src(j_decompress_ptr cinfo, JOCTET *pData, int FileSize, void *pDataSrc, JMETHOD(void, notifyCppWorld, (j_common_ptr))) {
   ...
   */
  if ( cinfo->progress == NULL) {   /* first time and progress notification wanted? */
      cinfo->progress = (struct jpeg_progress_mgr *)
        cinfo->mem->alloc_small((j_common_ptr) cinfo, JPOOL_PERMANENT,sizeof(struct jpeg_progress_mgr));
