...
/--------------------------------------------------------------------
|
|      $Id: plwinbmp.cpp,v 1.5 2004/09/11 12:41:37 uzadow Exp $
...
PLWinBmp::PLWinBmp () : m_pBMI (NULL), m_pBits (NULL) {
...
// Legacy routine. Should not be used.
  ...
  if (pSrPLBmp->HasAlpha()) {
    ...
    for (int sy = miny; sy<maxy; sy++) { // For each line
      BYTE * pDest = m_pBits+DestLineLen*(y+sy)+x*4;
      BYTE * pSrc = pSrPLBmp->m_pBits+SrcLineLen*sy;

      for (int sx = minx; sx<maxx; sx++) { // For each pixel
        if (pPal) {
...
long PLWinBmp::GetMemUsed() {
  PLASSERT_VALID(this);
  return GetMemNeeded(GetWidth(), GetHeight(), m_pBMI->biBitCount)+sizeof(*this);
}
...
void PLWinBmp::CreateRes(HINSTANCE lh_ResInst, int ID) {
  ...
  hRsrc = FindResource(lh_ResInst,MAKEINTRESOURCE (ID),RT_BITMAP);
...
void PLWinBmp::CreateFromHBitmap(HBITMAP hBitMap) {
  ...
  rc = ::GetDIBits(hdc, hBitMap, 0, GetHeight(), GetBits(), (BITMAPINFO *)GetBMI(), DIB_RGB_COLORS);
  PLASSERT (rc);
  free (pBMI);
  ::ReleaseDC(NULL, hdc);
  PLASSERT_VALID (this);
...
SIZE PLWinBmp::GetSize() { // Returns size in pixels
  SIZE sz;
...
// Draw the DIB to a given DC.
void PLWinBmp::StretchDraw(HDC hDC, int x, int y, double Factor, DWORD rop /* = SRCCOPY */) {
...
// Draw the DIB to a given DC.
void PLWinBmp::StretchDraw (HDC hDC, int x, int y, int w, int h, DWORD rop /* = SRCCOPY */) {
  PLASSERT_VALID (this);
...
bool PLWinBmp::FromClipboard(UINT uFormat) {
  ...
  else // uFormat must be either CF_BITMAP or CF_DIB
    PLASSERT (false);
...
void PLWinBmp::CopyPalette(PLWinBmp *pSrPLBmp) {
  if(pSrPLBmp->GetBitsPerPixel() <= 8 && GetPalette() && pSrPLBmp->GetPalette()) {
    for (UINT i = 0; i < (UINT)pSrPLBmp->GetNumColors(); i++) {
	  BYTE * pbSrc = (BYTE *)(pSrPLBmp->GetPalette() + i);
	  BYTE * pbDest = (BYTE *)(GetPalette() + i);
...
// Assumes that no memory is allocated before the call.
  ...
  int MemNeeded = GetMemNeeded(pBMI->biWidth, pBMI->biHeight, pBMI->biBitCount);
#ifdef MAX_BITMAP_SIZE
  if (MemNeeded > MAX_BITMAP_SIZE)
    throw PLTextException(PL_ERRDIB_TOO_LARGE, "Bitmap size too large.\n");
#endif
...
// and returns a handle to this block.
  ...
  hCopy = (HANDLE) ::GlobalAlloc(GMEM_MOVEABLE | GMEM_DDESHARE, MemUsed);
  if (hCopy == NULL)
