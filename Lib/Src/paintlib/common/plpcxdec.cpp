...
void PLPCXDecoder::Open(PLDataSource *pDataSrc) {
  ...
  SetBmpInfo (PLPoint(Width, Height), PLPoint (m_PcxHeader.Hres, m_PcxHeader.Vres), PLPixelFormat::X8R8G8B8);
}
...
// bytesperline bytes
void PLPCXDecoder::PCX_PlanesToPixels(PLBYTE *pixels, PLBYTE *bitplanes, short bytesperline, short planes, short bitsperpixel) {
  int i, j;
  int npixels;
  PLBYTE * p;
  if (planes > 4) {
...
// image Height rows, with each row having planes image planes each
// bytesperline bytes
void PLPCXDecoder::PCX_UnpackPixels(PLBYTE *pixels, PLBYTE *bitplanes, short bytesperline, short planes, short bitsperpixel) {
  register int bits;
  if (planes != 1) {
