...
public:
  ...
  //! Copy constructor
  PLAnyBmp( const PLBmpBase &Orig);

  //! Copy constructor
  PLAnyBmp( const PLAnyBmp &Orig);

  //! Assignment operator.
  PLAnyBmp &operator=( const PLBmpBase &Orig);
  ...
  PLBYTE * GetPixels()  { return m_pBits; }

  //! Returns a raw pointer to the bitmap bits.
  const PLBYTE * GetPixels() const { return m_pBits; }
...
protected:
  ...
  // is allocated yet.
  virtual void internalCreate( PLLONG Width,PLLONG Height,const PLPixelFormat& pf);
...
// constructor anyway :-(.
inline PLAnyBmp::PLAnyBmp( const PLAnyBmp &Orig) : PLBmp() {
  internalCopy (Orig);
}

inline PLAnyBmp::PLAnyBmp( const PLBmpBase &Orig) : PLBmp () {
  internalCopy (Orig);
}

inline PLAnyBmp & PLAnyBmp::operator=( const PLBmpBase &Orig) {
  PLBmp::operator=(Orig);
  return *this;
}

inline PLAnyBmp & PLAnyBmp::operator=( const PLAnyBmp &Orig) {
  PLBmp::operator=(Orig);
  return *this;
}
