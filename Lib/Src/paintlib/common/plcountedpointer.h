...
template <class type> void PLCountedPointer<type>::decCount() {
  if (!--*pCount) {
    delete pBody;
    delete pCount;
  }
...
template <class type> class PLCountedArrayPointer {
public:
  explicit  PLCountedArrayPointer(type * pType = 0) : pBody(pType), pCount(new size_t(1)) {
  }
  PLCountedArrayPointer(const PLCountedArrayPointer & copy) : pBody(copy.pBody), pCount(copy.pCount) {
    incCount();
  }
  ~PLCountedArrayPointer() {
    decCount();
  }
...
template <class type> PLCountedArrayPointer<type>& PLCountedArrayPointer<type>::operator=(const PLCountedArrayPointer & rhs) {
  if(pBody != rhs.pBody) {
    decCount();
    pBody = rhs.pBody;
    pCount = rhs.pCount;
...
template <class type> void PLCountedArrayPointer<type>::decCount() {
  if (!--*pCount) {
    delete [] pBody;
    delete pCount;
  }
