...
PLTextException::PLTextException(int Code, const char *pszErr) {
#ifdef _WIN32
  SetErrorMode(0); // Restore normal error handling just in case file system error checking was disabled.
#endif
...
PLTextException::PLTextException(int Code, int MinorCode, const char *pszErr) {
#ifdef _WIN32
  SetErrorMode(0); // Restore normal error handling just in case file system error checking was disabled.
#endif
...
PLTextException::PLTextException(const PLTextException &src) {
  m_Code      = src.m_Code;
  m_MinorCode = src.m_MinorCode;
  m_pszErr    = new char[strlen(src.m_pszErr)+1];
  strcpy(m_pszErr, src.m_pszErr);
}

PLTextException &PLTextException::operator=(const PLTextException &src) {
  delete[] m_pszErr;
  m_Code      = src.m_Code;
  m_MinorCode = src.m_MinorCode;
  m_pszErr    = new char[strlen(src.m_pszErr)+1];
...
int PLTextException::GetCode() const {
  return m_Code;
}
...
PLTextException::operator const char*() const {
