...
/--------------------------------------------------------------------
|
|      $Id: plbmpbase.cpp,v 1.12 2004/10/02 22:23:12 uzadow Exp $
|
...
void createTrueColorCopy (PLBmp & DestBmp, const PLBmpBase &SrcBmp, DestPixelC Dummy);
...
template<class DestPixelC>
void createTrueColorCopy (PLBmpBase &DestBmp, const PLBmpBase &SrcBmp, DestPixelC Dummy) {
...
void PLBmpBase::SetAlphaChannel(PLBmpBase * pAlphaBmp) {
  ...
  PLPixel32 **pLineArray = GetLineArray32();
  PLBYTE **pAlphaLineArray = pAlphaBmp->GetLineArray();

  for (int y=0; y < GetHeight(); y++) {
    PLPixel32 *pLine = pLineArray[y];
    PLBYTE *pAlphaLine = pAlphaLineArray[y];
    for (int x=0; x < GetWidth(); x++) {
...
bool PLBmpBase::AlmostEqual( const PLBmpBase& Bmp, int epsilon) const {
...
void PLBmpBase::initLocals( PLLONG Width, PLLONG Height, const PLPixelFormat & pf) {
...
void PLBmpBase::create1BPPCopy (const PLBmpBase & rSrcBmp) {
  ...
  for(int y = 0; y < Height; y++) { // For each line
    ...
    for (int x = 0; x < Width; x++) { // For each pixel
      ...
      if(pRGBA[PL_RGBA_RED] < 128 && pRGBA[PL_RGBA_GREEN] < 128 && pRGBA[PL_RGBA_BLUE] < 128)
