...
public:
  ...
  virtual void MakeBmpFromFileW(const wchar_t *pszwFName,PLBmp *pBmp, const PLPixelFormat &pf = PLPixelFormat::DONTCARE, PLIProgressNotification *pProgNot = NULL);
#endif
  ...
  virtual void MakeBmpFromResource(HINSTANCE hInstResource, int ResourceID, PLBmp *pBmp, const PLPixelFormat &pf = PLPixelFormat::DONTCARE, const char *ResType = NULL, HMODULE hResModule = 0);
#endif
...
#ifdef _WIN32
  void OpenFileW(const wchar_t *pszwFName, PLIProgressNotification *pProgNot = NULL);
#endif
...
inline PLBYTE PLPicDecoder::ReadByte( PLDataSource * pDataSrc) {
  return *(pDataSrc->ReadNBytes(1));
}


inline PLWORD PLPicDecoder::ReadIWord( PLDataSource * pDataSrc) {
...
inline PLWORD PLPicDecoder::ReadMWord( PLDataSource * pDataSrc) {
  PLBYTE * pData = pDataSrc->Read2Bytes();
  return pData[1] + (pData[0]<<8);
}

inline PLLONG PLPicDecoder::ReadILong( PLDataSource * pDataSrc) {
  PLBYTE * pData = pDataSrc->Read4Bytes();
  return pData[0] + (pData[1]<<8) + (pData[2]<<16) + (pData[3]<<24);
}

inline PLLONG PLPicDecoder::ReadMLong( PLDataSource * pDataSrc) {
  PLBYTE * pData = pDataSrc->Read4Bytes();
