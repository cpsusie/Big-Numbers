...
/////////////////////////////////////////////////////////////////////
// PLBmp creation
void PLBmp::Create(PLLONG Width, PLLONG Height, const PLPixelFormat &pf, PLBYTE *pBits, int Stride, const PLPoint &Resolution) {
  PLASSERT_VALID(this);
...
void PLBmp::Create(const PLBmpInfo &Info) {
  Create(Info.GetWidth(), Info.GetHeight(), Info.GetPixelFormat(), NULL, 0, Info.GetResolution());
}

void PLBmp::CreateCopy(const PLBmpBase &rSrcBmp, const PLPixelFormat &pfWanted) {
  PLASSERT_VALID (&rSrcBmp);

  int BPPWanted = pfWanted.GetBitsPerPixel();

  PLASSERT(BPPWanted == 32 || BPPWanted == 24 || BPPWanted == 16 || BPPWanted == 8 || BPPWanted == 1 || BPPWanted == 0);
  int BPPSrc = rSrcBmp.GetBitsPerPixel();
  PLASSERT(BPPSrc == 32 || BPPSrc == 24 || BPPSrc == 16 || BPPSrc == 8 || BPPSrc == 1);

  if (pfWanted == rSrcBmp.GetPixelFormat() || pfWanted == PLPixelFormat::DONTCARE) {
    if(&rSrcBmp == this)
      return;
    // Create empty bitmap.
    freeMembers ();
    ...
    Create (rSrcBmp.GetWidth(), rSrcBmp.GetHeight(),pfWanted);
  }
  CopyPixels(rSrcBmp);
...
void PLBmp::initLocals (PLLONG Width, PLLONG Height, const PLPixelFormat& pf) {
  SetBmpInfo(PLPoint (Width, Height), PLPoint(0,0), pf);
  initLineArray();

  if (GetBitsPerPixel() <= 8) { // Color table exists
    SetGrayPalette ();
  }
...
void PLBmp::internalCopy(const PLBmpBase & rSrcBmp) {
  PLASSERT_VALID(&rSrcBmp);
  // Create empty bitmap.
  internalCreate (rSrcBmp.GetWidth(), rSrcBmp.GetHeight(), rSrcBmp.GetPixelFormat());
