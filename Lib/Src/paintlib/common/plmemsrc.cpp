...
PLMemSource::PLMemSource() : PLDataSource(NULL), m_pCurPos(NULL), m_pStartPos(0) {
// No progress notification nessesary when
// reading from memory.
}
...
PLBYTE * PLMemSource::ReadNBytes(int n) {
...
void PLMemSource::Seek(int n) {
  m_pCurPos = m_pStartPos+n;
}

// Read but don't advance file pointer.
PLBYTE * PLMemSource::GetBufferPtr(int MinBytesInBuffer) {
  return m_pCurPos;
}
