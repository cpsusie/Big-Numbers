...
inline PLRect::PLRect(const PLPoint& TL, const PLPoint& BR): tl(TL), br(BR) {
}

inline PLRect::PLRect(int left,int top,int right,int bottom) : tl(left, top), br (right, bottom) {
}

inline bool PLRect::operator==( const PLRect & rect) const {
...
inline int PLRect::Width() const {
  return br.x-tl.x;
}

inline int PLRect::Height() const {
  return br.y-tl.y;
}

inline void PLRect::SetWidth(int width) {
    br.x = tl.x+width;
}
 
inline void PLRect::SetHeight(int height) {
    br.y = tl.y+height;
}

inline bool PLRect::Contains( const PLPoint& pt) const {
    return pt.x >= tl.x && pt.x < br.x && pt.y >= tl.y && pt.y < br.y;
}

inline bool PLRect::Contains( const PLRect& rect) const {
    PLPoint brpt(rect.br.x-1, rect.br.y-1);
    return Contains(rect.tl) && Contains(brpt);
}

inline bool PLRect::Intersects( const PLRect& rect) const {   
    if(rect.br.x <= tl.x || rect.tl.x >= br.x || rect.br.y <= tl.y || rect.tl.y >= br.y)
      return false;
    else
      return true;
}
...
inline void PLRect::Expand( const PLRect& rect) {
...
inline void PLRect::Intersect( const PLRect& rect) {
