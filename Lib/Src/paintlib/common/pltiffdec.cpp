...
/--------------------------------------------------------------------
|
|      $Id: pltiffdec.cpp,v 1.18 2004/11/09 15:55:06 artcom Exp $
|      TIFF Decoder Class
|
...
PLTIFFDecoder::PLTIFFDecoder() : PLPicDecoder(), m_pTif(0) {
}
...
void PLTIFFDecoder::Open(PLDataSource *pDataSrc) {
  PLASSERT (m_pTif == 0);
  m_pTif = TIFFOpenMem (pDataSrc->ReadEverything(), pDataSrc->GetFileSize(), NULL);
  if (!m_pTif)
...
void PLTIFFDecoder::GetImage(PLBmpBase &Bmp) {
  ...
  if ((SamplePerPixel == 1 || SamplePerPixel == 2) && BitsPerSample <= 8 &&  !TIFFIsTiled(m_pTif))
...
void PLTIFFDecoder::doLoColor (TIFF * tif, PLBmpBase * pBmp) {
  ...
  pBmp->SetPalette(pPal); // propagate colormap
  delete [] pBits; // clean up
}
