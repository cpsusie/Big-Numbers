...
inline PLPoint::PLPoint(int X, int Y) {
  x = X;
  y = Y;
}

inline bool PLPoint::operator==(const PLPoint & pt) const {
...
inline void PLPoint::operator+=(const PLPoint & pt) {
  x += pt.x;
  y += pt.y;
}
...
inline PLPoint PLPoint::operator-() const {
  return PLPoint(-x, -y);
}

inline PLPoint PLPoint::operator+(const PLPoint & pt) const {
...
inline PLPoint PLPoint::operator/( double f) const {
