...
void PLPicDecoder::MakeBmpFromFile(const char *pszFName, PLBmp *pBmp, const PLPixelFormat &pfWanted, PLIProgressNotification *pProgNot) {
  try {
...
// Decodes a picture in a file by creating a file data source and
// calling MakeBmp with this data source.
void PLPicDecoder::MakeBmpFromFileW(const wchar_t *pszwFName, PLBmp *pBmp, const PLPixelFormat &pfWanted, PLIProgressNotification * pProgNot) {
  try {
...
void PLPicDecoder::MakeBmpFromURL(const char *pszURL, PLBmp *pBmp, const PLPixelFormat &pfWanted, PLIProgressNotification * pProgNot) {
#ifndef PL_SUPPORT_CURL
  throw PLTextException(PL_ERRFORMAT_NOT_COMPILED, "URL support not compiled into library.");
#else
  char sz[256];
  sprintf (sz, "--- Decoding URL %s. ---\n", pszURL);
  Trace (1, sz);

  PLURLSource *pSrc = NULL;
  try {
    pSrc = new PLURLSource();
    m_pDataSrc = pSrc;
    int err = pSrc->Open(pszURL);
    if(err) {
...
void PLPicDecoder::MakeBmpFromResource(HINSTANCE hInstResource, int ResourceID, PLBmp *pBmp, const PLPixelFormat &pfWanted, const char *ResType, HMODULE hResModule) {
  char sz[256];
  ...
  try {
    ...
    m_pDataSrc = pResSrc;
    int err = pResSrc->Open(hInstResource, ResourceID, ResType);
    if(err) {
...
void PLPicDecoder::MakeBmpFromMemory(unsigned char *ucMemSrc,int MemSrcSize, PLBmp *pBmp, const PLPixelFormat &pfWanted, PLIProgressNotification * pProgNot) {
  ...
  try {
    ...
    int err = pMemSrc->Open(ucMemSrc, MemSrcSize);
    if(err) {
...
void PLPicDecoder::MakeBmp(PLBmp *pBmp, const PLPixelFormat &pfWanted) {
  PLBmp *pTempBmp = NULL;

  if(pfWanted == PLPixelFormat::DONTCARE || pfWanted == GetPixelFormat()) {
    pTempBmp = pBmp;
...
void PLPicDecoder::SetTraceConfig(int Level, char *pszFName) {
...
#ifdef _WIN32
void PLPicDecoder::OpenFileW(const wchar_t *pszwFName, PLIProgressNotification *pProgNot) {
  char sz[1024];
  char sz2[900];

  // First calculate how much room the result will take up
  int neededSize = ::WideCharToMultiByte(CP_ACP,0,pszwFName,-1,NULL,0,NULL,NULL);
  if(neededSize < 900){  // Prevent overflow of our buffer
    ::WideCharToMultiByte(CP_ACP,0,pszwFName,-1,sz2,900,NULL,NULL);
  } else {
  ...
  PLFileSource *pFileSrc = NULL;
  try {
    pFileSrc = new PLFileSource(pProgNot);
    int err = pFileSrc->OpenW(pszwFName);
    if(err) {
...
void PLPicDecoder::OpenFile(const char *pszFName, PLIProgressNotification *pProgNot) {
  char sz[1024];
  sprintf (sz, "--- Decoding file %s. ---\n", pszFName);
  Trace (1, sz);
  ...
  try {
    pFileSrc = new PLFileSource(pProgNot);
    int err = pFileSrc->Open(pszFName);
    if(err) {
...
void PLPicDecoder::raiseError(int Code, char *pszErr) {
  char sz[256];
...
void PLPicDecoder::Trace(int TraceLevel, const char *pszMessage) {
  if(TraceLevel <= m_TraceLevel) {
    if(m_pszTraceFName) {
      ...
      if(pFile != NULL)  {
        ...
        fclose(pFile);
      } else { // No permission? File locked? Filename nonsense?
...
PLBYTE * PLPicDecoder::unpackPictRow(PLBYTE *pLineBuf, PLDataSource *pDataSrc, int Width, int rowBytes, int SrcBytes) {
    ...
    for(int j = 0; j < SrcBytes; ) {
      ...
      if(FlagCounter & 0x80) {
        if(FlagCounter == 0x80) {
          ...
          const int len = ((FlagCounter ^ 255) & 255) + 2;
          memset(pCurPixel, *(pSrcLine+j+1), len);
          pCurPixel += len;
          j += 2;
        }
      } else { // Unpacked data
        const int len = (FlagCounter & 255) + 1;
