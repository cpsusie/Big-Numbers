...
/--------------------------------------------------------------------
|
|      $Id: pljpegdec.cpp,v 1.13 2004/09/11 12:41:35 uzadow Exp $
|      JPEG Decoder Class
|
...
METHODDEF(void) error_exit(j_common_ptr pcinfo) { // This procedure is called by the IJPEG library when an error occurs.
...
// Creates a decoder
PLJPEGDecoder::PLJPEGDecoder() : m_bFast(true), m_pExifData(NULL) {
...
void PLJPEGDecoder::decodeRGB( PLBmpBase * pBmp) { // Assumes IJPEG decoder is already set up.
  int CurLine = 0;
...
void PLJPEGDecoder::decodeGray(PLBmpBase *pBmp) { // Assumes IJPEG decoder is already set up.
  PLBYTE * pDst;
  int CurLine = 0;
