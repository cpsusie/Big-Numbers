...
// Creates a decoder
PLIFF85Decoder::PLIFF85Decoder() : m_formType(0), m_bitmapHeader(), m_pal(), m_viewMode(0) {
}
...
void PLIFF85Decoder::Open(PLDataSource *pDataSrc) {
  ...
  if (PLIFF85::MakeID(reinterpret_cast<const char*> (pDataSrc->GetBufferPtr(sizeof chunk))) != PLIFF85::ID_FORM) {
...
void PLIFF85Decoder::GetImage(PLBmpBase & Bmp) {
  ...
  for (int row = 0; row < m_bitmapHeader.h; row++) {
    ...
    if ((m_viewMode & PLIFF85::viewHAM) != 0) {
      ...
      for (int i = 0; i < m_bitmapHeader.w; i++) {
        ...
        switch (mod) {
          ...
          case 1:
            ...
            pThePixel->Set(prevPixel.GetR(),prevPixel.GetG(),static_cast<PLBYTE> (the_byte << 4),prevPixel.GetA());
            break;
          case 2:
            // Hold previous, replacing Red component with bits from planes 0-3.
            pThePixel->Set(static_cast<PLBYTE> (the_byte << 4),prevPixel.GetG(),prevPixel.GetB(),prevPixel.GetA());
            break;
          case 3:
            // Hold previous, replacing Green component with bits from planes 0-3.
            pThePixel->Set(prevPixel.GetR(),static_cast<PLBYTE> (the_byte << 4),prevPixel.GetB(),prevPixel.GetA());
            break;
          default:
      ...
      std::copy(decodedBuf.begin(),decodedBuf.end(),Bmp.GetLineArray()[row]);
    }
  }
} // GetImage
