...
METHODDEF(void) jpeg_mem_dest_init(j_compress_ptr cinfo) {
  ...
  dest->buffer = (JOCTET *)cinfo->mem->alloc_small((j_common_ptr) cinfo, JPOOL_IMAGE,JPEG_WORK_BUFFER_SIZE);
...
METHODDEF(boolean) jpeg_mem_dest_empty_output_buffer (j_compress_ptr cinfo) {
  ...
  if (dest->pDataSink->WriteNBytes(JPEG_WORK_BUFFER_SIZE, dest->buffer) != (size_t) JPEG_WORK_BUFFER_SIZE)
 ...
 *
 */
GLOBAL(void) jpeg_mem_dest(j_compress_ptr cinfo, JOCTET * pData, int FileSize, PLDataSink * pDataSink) {
  datasink_dest_mgr *dest;

  if(cinfo->dest == NULL) {   /* first time for this JPEG object? */
    cinfo->dest = (struct jpeg_destination_mgr *)
                  cinfo->mem->alloc_small((j_common_ptr)cinfo, JPOOL_PERMANENT,sizeof(datasink_dest_mgr));
  }
