...
/--------------------------------------------------------------------
|
|      $Id: plsdlbmp_8h-source.html,v 1.3 2004/07/28 15:15:29 uzadow Exp $
|
...
// constructor anyway :-(.
inline PLSDLBmp::PLSDLBmp( const PLSDLBmp &Orig) : PLBmp () {
  m_SurfFlags = Orig.m_SurfFlags;
  internalCopy (Orig);
}

inline PLSDLBmp::PLSDLBmp( const PLBmpBase &Orig) : PLBmp () {
  m_SurfFlags = SDL_SWSURFACE;
  internalCopy (Orig);
}

inline PLSDLBmp & PLSDLBmp::operator=( const PLBmpBase &Orig) {
  PLBmp::operator=(Orig);
  return *this;
}

inline PLSDLBmp & PLSDLBmp::operator=( const PLSDLBmp &Orig) {
  m_SurfFlags = Orig.m_SurfFlags;
  PLBmp::operator=(Orig);
  return *this;
}
...
/--------------------------------------------------------------------
|
|      $Log: plsdlbmp_8h-source.html,v $
|      Revision 1.3  2004/07/28 15:15:29  uzadow
