...
void PLDataSink::Open(const char *pszName, PLBYTE *pData, size_t MaxFileSize) {
...
void PLDataSink::OpenW(const wchar_t *pszwName, PLBYTE *pData, size_t MaxFileSize) {
  // Data source may not be open already!
  PLASSERT (! m_pStartData);
  PLASSERT (MaxFileSize > 0);
...
size_t PLDataSink::WriteNBytes(size_t n, PLBYTE *pData) {
