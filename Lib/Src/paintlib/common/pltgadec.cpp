...
void PLTGADecoder::Open(PLDataSource *pDataSrc) {
  ...
  SetBmpInfo(PLPoint (m_TgaHead.ImageWidth, m_TgaHead.ImageHeight), PLPoint(0, 0), pf);
}

void PLTGADecoder::GetImage (PLBmpBase &Bmp) { // Read the color map data (Version 1.0 and 2.0).
...
void PLTGADecoder::readTgaHeader(TGAHEADER *pTgaHead, PLDataSource *pDataSrc) {
...
void PLTGADecoder::readImage(TGAHEADER *pTgaHead, PLBmpBase *pBmp, PLDataSource *pDataSrc) {
  bool bCompressed;

  if (pTgaHead->ImageType == TGA_Mono ||  pTgaHead->ImageType == TGA_RLEMono)
    pBmp->SetGrayPalette();
...
void PLTGADecoder::readData(TGAHEADER *pTgaHead, bool bCompressed, PLBmpBase *pBmp, PLDataSource *pDataSrc) {
...
void PLTGADecoder::expandUncompressedLine(PLBYTE *pDest, int Width, bool bReversed, int bpp, PLDataSource *pDataSrc) {
...
void PLTGADecoder::expandCompressedLine(PLBYTE *pDest, int Width, bool bReversed, int bpp, PLDataSource *pDataSrc) {
  PLBYTE Count;

  for(int x=0; x<Width; ) {
    Count = ReadByte (pDataSrc);
    if (Count & 128) { // RLE-Encoded packet
...
PLPixel32 PLTGADecoder::readPixel32( int bpp, PLDataSource * pDataSrc)
...
PLBYTE PLTGADecoder::readPixel8(int bpp, PLDataSource * pDataSrc) {
  PLASSERT (bpp == 8);
