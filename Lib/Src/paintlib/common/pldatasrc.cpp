...
PLDataSource::PLDataSource(PLIProgressNotification *pNotification)
    : m_pszName(NULL),
#ifdef _WIN32
        m_pszwName(NULL),
        m_bNameIsWide(false),
...
void PLDataSource::Open(const char *pszName, int FileSize) {
...
#ifdef _WIN32
void PLDataSource::OpenW(const wchar_t * pszwName,  int FileSize) {
  // Data source may not be open already!
  PLASSERT (!m_FileSize);
...
PLBYTE * PLDataSource::ReadNBytes( int n) {
