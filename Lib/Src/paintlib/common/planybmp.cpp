...
/--------------------------------------------------------------------
|
|      $Id: planybmp.cpp,v 1.9 2004/09/11 12:41:34 uzadow Exp $
...
PLAnyBmp::PLAnyBmp() : m_pBits(NULL) { // Creates an empty bitmap.
  internalCreate(16, 16, PLPixelFormat::I8);
  PLASSERT_VALID(this);
}

PLAnyBmp::~PLAnyBmp() { // Free the memory.
  freeMembers();
}

long PLAnyBmp::GetMemUsed() { // Returns the memory used by the object.
  PLASSERT_VALID(this);

  return GetMemNeeded (GetWidth(), GetHeight(), GetBitsPerPixel())+ sizeof (*this);
}

long PLAnyBmp::GetBytesPerLine() { // Returns number of bytes used per line.
  // bdelmee code change
...
long PLAnyBmp::GetBitsMemNeeded(PLLONG width, PLLONG height, PLWORD BitsPerPixel) {
  int LineMem = width*BitsPerPixel/8; // Calculate memory per line.

  // bdelmee code change
  if (BitsPerPixel == 1 && width % 8)
    LineMem++;

  return LineMem*height; // Multiply by number of lines
}
...
void PLAnyBmp::internalCreate(PLLONG Width, PLLONG Height, const PLPixelFormat &pf) {
  // Allocate memory
