...
void AbstractAxisPainter::paintXAxis() {
  ...
  line(left , origY, right, origY);
  line(right, origY, right - ARROW_SIZE, origY - ARROW_SIZE / 2);
  line(right, origY, right - ARROW_SIZE, origY + ARROW_SIZE / 2);
}
...
void AbstractAxisPainter::paintYAxis() {
  ...
  line(origX, top, origX, bottom);
  line(origX, top, origX - ARROW_SIZE / 2, top + ARROW_SIZE);
  line(origX, top, origX + ARROW_SIZE / 2, top + ARROW_SIZE);
}
...
void AbstractAxisPainter::xTextOut(double x, const String &s, int yOffset) {
  ...
  CSize dim = getTextExtent(s);
  int xp = (int)x + ((((int)round(x) == origin.x || m_grid) && m_yRectInterval.getMax() > origin.y) ? 1 : -dim.cx/2);
  yOffset += PIN_LENGTH + 3;
  int yp = origin.y + yOffset;
  textOut(xp, yp, s);
  if(!isPainting()) {
    int m = (int)fabs(yOffset);
...
void AbstractAxisPainter::yTextOut(double y, const String &s, int xOffset) {
  ...
  int xp = origin.x + xOffset;
  int yp = (int)(y - ((((int)round(y) == origin.y || m_grid) && m_xRectInterval.getMin() < origin.x) ? dim.cy : dim.cy/2));
  textOut(xp, yp, s);
  if(!isPainting()) {
    int m = (int)fabs(xOffset);
    if(m > m_maxTextOffset) {
      m_maxTextOffset = m;
    }
  }
...
void AbstractAxisPainter::textOut(int x, int y, const String &txt) {
  if(!isPainting()) {
    return;
  }
  CDC *dc = m_vp->getDC();
...
void AbstractAxisPainter::paintHorizontalPin(double y, bool big) {
  ...
  if(big) {
    if(m_grid) {
      ...
      line(origin.x,y, origin.x - PIN_LENGTH*2,y);
    }
  } else {
    line(origin.x,y, origin.x - PIN_LENGTH,y);
  }
}
...
void AbstractAxisPainter::paintVerticalPin(double x, bool big) {
  ...
  if(big) {
    if(m_grid) {
      ...
      line(x,origin.y, x, origin.y + PIN_LENGTH*2);
    }
  } else {
    line(x, origin.y, x, origin.y + PIN_LENGTH);
  }
}

void AbstractAxisPainter::verticalGridLine(double x) {
  line(x, m_yRectInterval.getMin(), x, m_yRectInterval.getMax(), m_gridPen);
}

void AbstractAxisPainter::horizontalGridLine(double y) {
  line(m_xRectInterval.getMin(), y, m_xRectInterval.getMax(), y, m_gridPen);
}

void AbstractAxisPainter::line(double x1, double y1, double x2, double y2) {
  if(!isPainting()) {
    return;
  }
  line(x1, y1, x2, y2, m_solidPen);
}

void AbstractAxisPainter::line(double x1, double y1, double x2, double y2, CPen &pen) {
  if(!isPainting()) {
    return;
  }
  CDC *dc = m_vp->getDC();
  dc->SelectObject(&pen);
  dc->MoveTo((int)round(x1), (int)round(y1));
  dc->LineTo((int)round(x2), (int)round(y2));
}
