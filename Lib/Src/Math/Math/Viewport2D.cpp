...
void Viewport2D::MoveTo(const Point2D &p) {
  m_dc->MoveTo(m_tr->forwardTransform(p));
}

void Viewport2D::MoveTo(double x, double y) {
  m_dc->MoveTo(m_tr->forwardTransform(x,y));
}

void Viewport2D::LineTo(const Point2D &p) {
  m_dc->LineTo(m_tr->forwardTransform(p));
}

void Viewport2D::LineTo(double x, double y) {
  m_dc->LineTo(m_tr->forwardTransform(x,y));
}

void Viewport2D::SetPixel(const Point2D &p, COLORREF color) {
  m_dc->SetPixel(m_tr->forwardTransform(p),color);
}

void Viewport2D::SetPixel(double x, double y, COLORREF color) {
  m_dc->SetPixel(m_tr->forwardTransform(x,y),color);
}

COLORREF Viewport2D::GetPixel(const Point2D &p) {
  return m_dc->GetPixel(m_tr->forwardTransform(p));
}

COLORREF Viewport2D::GetPixel(double x, double y) {
  return m_dc->GetPixel(m_tr->forwardTransform(x,y));
}
...
void Viewport2D::paintCross(const Point2D &point, COLORREF color, int size) {
  CPoint p = (CPoint)m_tr->forwardTransform(point);
  if(size%1) {
...
bool Viewport2D::Rectangle(const Rectangle2D &r) {
  CRect tmp = m_tr->forwardTransform(r);
...
void Viewport2D::FillSolidRect(const Rectangle2D &r, COLORREF color) {
  CRect rect = m_tr->forwardTransform(r);
...
void Viewport2D::paintDragRect(const Rectangle2D &rect, SIZE size, const Rectangle2D &lastRect, SIZE lastSize, CBrush *brush, CBrush *lastBrush) {
  CRect r     = m_tr->forwardTransform(rect);
  CRect lastr = m_tr->forwardTransform(lastRect);
