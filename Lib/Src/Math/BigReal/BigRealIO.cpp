...
void BigReal::formatScientific(String &result, int precision, long flags, int expo10, bool removeTrailingZeroes) const {
  ...
  result += format(_T("%+04d"), BigReal::getExpo10(nn));
}
...
BigRealStream &operator<<(BigRealStream &stream, const BigReal &x) {
  ...
  try {
    ...
    int missing = stream.getWidth() - result.length();
    if(missing <= 0) {
      stream.append(result);
    } else {
      if(flags & ios::left) {
        stream.append(result).append(spaceString(missing));
      } else { // right align
        stream.append(spaceString(missing)).append(result);
      }
    }
  } catch(Exception e) {
...
#define appendCharGetNext(in, ch) { in >> ch; buf += String(ch); peekChar(in,ch); }
