...
template<class T> void MemoryPool<T>::poolinit(int bytecount, int itemcount, int firstitemcount, int alignment) {
  ...
  firstblock = (VOID **)trimalloc(itemsfirstblock * getItemBytes() + (int) sizeof(VOID *) + alignbytes);
  ...
  if (deaditemstack != NULL) {
    ...
    if (unallocateditems == 0) {
      ...
      if (*(nowblock) == NULL) {
        ...
        newblock = (VOID **) trimalloc(itemsperblock * getItemBytes() +
                                       (int) sizeof(VOID *) +
                                       alignbytes);
...
public:
  void       printtriangle(        Behavior  *b, otri *t);
  void       printsubseg(          Behavior  *b, osub *s);
  void       dummyinit(            Behavior  *b, int trianglebytes, int subsegbytes);
  void       initializevertexpool( Behavior  *b);
  void       initializetrisubpools(Behavior  *b);
  ...
  vertex     getvertex(            Behavior   *b, int number);
  void       triangleinit();
  void       triangledeinit(       Behavior   *b);
  void       maketriangle(         Behavior   *b, otri *newotri);
  void       makesubseg(           osub       *newsubseg);
  REAL       counterclockwise(     Behavior   *b, vertex pa, vertex pb, vertex pc);
  REAL       incircle(             Behavior   *b, vertex pa, vertex pb, vertex pc, vertex pd);
  REAL       orient3d(             Behavior   *b,
                                   vertex      pa
                                  ,vertex      pb
                                  ,vertex      pc
                                  ,vertex      pd
                                  ...
                                  ,REAL        cheight
                                  ,REAL        dheight);
  REAL         nonregular(         Behavior   *b
                                  ,vertex      pa
                                  ,vertex      pb
                                  ,vertex      pc
                                  ,vertex      pd);
  void         findcircumcenter(   Behavior   *b,
                                   vertex      torg
                                  ,vertex      tdest
                                  ,vertex      tapex
                                  ,vertex      circumcenter
                                  ...
                                  ,REAL       *eta
                                  ,int         offcenter);
  void         checkmesh(          Behavior   *b);
  void         checkdelaunay(      Behavior   *b);
  void         enqueuebadtriang(   Behavior   *b, BadTriangle *badtri);
  void         enqueuebadtri(      Behavior   *b
                                  ,otri       *enqtri
                                  ,REAL        minedge
                                  ,vertex      enqapex
                                  ,vertex      enqorg
  ...
  int          checkseg4encroach(  Behavior   *b, osub *testsubseg);
  void         testtriangle(       Behavior   *b, otri *testtri);

  void         makevertexmap(      Behavior   *b);
  LocateResult preciselocate(      Behavior   *b, vertex searchpoint, otri *searchtri, int stopatsubsegment);
  LocateResult locate(             Behavior   *b, vertex searchpoint, otri *searchtri);

  void insertsubseg(               Behavior   *b, otri *tri, int subsegmark);
  void flip(                       Behavior   *b, otri *flipedge);
  void unflip(                     Behavior   *b, otri *flipedge);
  InserVvertexResult insertvertex( Behavior   *b
                                  ,vertex      newvertex
                                  ,otri       *searchtri
                                  ,osub       *splitseg
                                  ,int         segmentflaws
  ...
  void triangulatepolygon(         Behavior   *b
                                  ,otri       *firstedge
                                  ,otri       *lastedge
                                  ,int         edgecount
                                  ,int         doflip
                                  ,int         triflaws);
  void deletevertex(               Behavior   *b
                                  ,otri       *deltri);
  void undovertex(                 Behavior   *b);
  void mergehulls(                 Behavior   *b
                                  ,otri       *farleft
                                  ,otri       *innerleft
                                  ,otri       *innerright
                                  ,otri       *farright
  ...
  void divconqrecurse(             Behavior   *b
                                  ,vertex     *sortarray
                                  ,int         vertices
                                  ,int         axis
                                  ,otri       *farleft
                                  ,otri       *farright);
  long removeghosts(               Behavior   *b
                                  ,otri       *startghost);
  long divconqdelaunay(            Behavior   *b);
  void boundingbox(                Behavior   *b);
  long removebox(                  Behavior   *b);
  long incrementaldelaunay(        Behavior   *b);
  ...
  SplayNode *splayinsert(          SplayNode  *splayroot
                                  ,otri       *newkey
                                  ,vertex      searchpoint);
  SplayNode *circletopinsert(      Behavior   *b,
                                   SplayNode  *splayroot
                                  ,otri       *newkey
                                  ,vertex      pa
                                  ,vertex      pb
  ...
  SplayNode *frontlocate(          SplayNode  *splayroot
                                  ...
                                  ,otri       *searchtri
                                  ,int        *farright);
  long sweeplinedelaunay(          Behavior   *b);
  long delaunay(                   Behavior   *b);

  FindDirectionResult finddirection(Behavior  *b, otri *searchtri, vertex searchpoint);
  void segmentintersection(        Behavior   *b, otri *splittri, osub *splitsubseg, vertex endpoint2);
  int scoutsegment(                Behavior   *b, otri *searchtri, vertex endpoint2, int newmark);
  void conformingedge(             Behavior   *b, vertex endpoint1, vertex endpoint2, int newmark);
  void delaunayfixup(              Behavior   *b, otri *fixuptri, int leftside);
  void constrainededge(            Behavior   *b, otri *starttri, vertex endpoint2, int newmark);
  void insertsegment(              Behavior   *b, vertex endpoint1, vertex endpoint2, int newmark);
  void markhull(                   Behavior   *b);

  void infecthull(                 Behavior   *b);
  void plague(                     Behavior   *b);
  void regionplague(               Behavior   *b, REAL attribute, REAL area);
  void carveholes(                 Behavior   *b, REAL *holelist, int holes, REAL *regionlist, int regions);
  void tallyencs(                  Behavior   *b);
  void splitencsegs(               Behavior   *b, int triflaws);
  void tallyfaces(                 Behavior   *b);
  void splittriangle(              Behavior   *b, BadTriangle *badtri);
  void enforcequality(             Behavior   *b);
  void highorder(                  Behavior   *b);


#ifdef TRILIBRARY
  int reconstruct(                 Behavior   *b
                                  ,int        *trianglelist
  ...
  void formskeleton(               Behavior   *b
                                  ,int        *segmentlist
                                  ,int        *segmentmarkerlist
                                  ,int         numberofsegments);
  void transfernodes(              Behavior   *b
                                  ,REAL       *pointlist
                                  ,REAL       *pointattriblist
                                  ,int        *pointmarkerlist
                                  ,int         numberofpoints
                                  ,int         numberofpointattribs);
  void writenodes(                 Behavior   *b
                                  ,REAL      **pointlist
                                  ,REAL      **pointattriblist
                                  ,int       **pointmarkerlist);
  void writeelements(              Behavior   *b
                                  ,int       **trianglelist
                                  ,REAL      **triangleattriblist);
  void writepoly(                  Behavior   *b
                                  ,int       **segmentlist
                                  ,int       **segmentmarkerlist);
  void writeedges(                 Behavior   *b
                                  ,int       **edgelist
                                  ,int       **edgemarkerlist);
  void writevoronoi(               Behavior   *b
                                  ,REAL      **vpointlist
                                  ...
                                  ,int       **vedgemarkerlist
                                  ,REAL      **vnormlist);
  void writeneighbors(             Behavior   *b
                                  ,int       **neighborlist);
#else /* not TRILIBRARY */
  long reconstruct(                Behavior   *b
                                  ,char       *elefilename
                                  ,char       *areafilename
                                  ,char       *polyfilename
                                  ,FILE       *polyfile);
  void formskeleton(               Behavior   *b
                                  ,FILE       *polyfile
                                  ,char       *polyfilename);
  void readnodes(                  Behavior   *b
                                  ,char       *nodefilename
                                  ,char       *polyfilename
                                  ,FILE      **polyfile);
  void readholes(                  Behavior   *b,
                                   FILE       *polyfile
                                  ,char       *polyfilename
  ...
  void writeneighbors(             Behavior   *b
                                  ...
                                  ,int         argc, char **argv);
  void writeoff(                   Behavior   *b
                                  ,char       *offfilename
                                  ,int         argc, char **argv);
#endif /* not TRILIBRARY */

  void numbernodes(                Behavior   *b);
  void quality_statistics(         Behavior   *b);
  void statistics(                 Behavior   *b);
      ...
      for (j = STARTINDEX; argv[i][j] != '\0'; j++) {
        ...
        if (argv[i][j] == 'q') {
          ...
          if (((argv[i][j + 1] >= '0') && (argv[i][j + 1] <= '9')) ||
            ...
            workstring[k] = '\0';
            b->minangle = (REAL) strtod(workstring, NULL);
	  } else {
            b->minangle = 20.0;
	  }
	}
        if (argv[i][j] == 'a') {
          ...
          if (((argv[i][j + 1] >= '0') && (argv[i][j + 1] <= '9')) ||
            ...
            workstring[k] = '\0';
            b->maxarea = (REAL) strtod(workstring, NULL);
            if (b->maxarea <= 0.0) {
...
/*                                                                           */
/*****************************************************************************/
void Mesh::printtriangle(Behavior *b, otri *t) {
  otri printtri;
  osub printsh;
  vertex printvertex;
...
void Mesh::printsubseg(Behavior *b, osub *s) {
  osub printsh;
  otri printtri;
  vertex printvertex;
...
void Mesh::dummyinit(Behavior *b, int trianglebytes, int subsegbytes) {
  unsigned long alignptr;

  /* Set up `dummytri', the `Triangle' that occupies "outer space." */
  dummytribase = (Triangle *) trimalloc(trianglebytes +
                                           triangles.getAlignBytes());
  /* Align `dummytri' on a `triangles.alignbytes'-byte boundary. */
  alignptr = (unsigned long) dummytribase;
  dummytri = (Triangle *)
    (alignptr + (unsigned long) triangles.getAlignBytes() -
     (alignptr % (unsigned long) triangles.getAlignBytes()));
  ...
  if (b->usesegments) {
    ...
    alignptr = (unsigned long) dummysubbase;
    dummysub = (SubSeg *) (alignptr + (unsigned long) subsegs.getAlignBytes()
                           - (alignptr % (unsigned long) subsegs.getAlignBytes()));
...
/*                                                                           */
/*****************************************************************************/
void Mesh::initializevertexpool(Behavior *b) {
  int vertexsize;
...
/*                                                                           */
/*****************************************************************************/
void Mesh::initializetrisubpools(Behavior *b) {
  int trisize;
  ...
  trisize = ((b->order + 1) * (b->order + 2) / 2 + (highorderindex - 3)) *
            sizeof(Triangle);
  ...
  areaboundindex = elemattribindex + eextras + b->regionattrib;
  /* If Triangle attributes or an area bound are needed, increase the number */
  /*   of bytes occupied by a Triangle.                                      */
  if (b->vararea) {
    trisize = (areaboundindex + 1) * sizeof(REAL);
  } else if (eextras + b->regionattrib > 0) {
  ...
  if (b->usesegments) {
    ...
    dummyinit(b, triangles.getItemBytes(), subsegs.getItemBytes());
  } else {
    /* Initialize the "outer space" Triangle. */
    dummyinit(b, triangles.getItemBytes(), 0);
  }
}
...
/*                                                                           */
/*****************************************************************************/
vertex Mesh::getvertex(Behavior *b, int number) {
  VOID **getblock;
  char *foundvertex;
  unsigned long alignptr;
  int current;
  ...
  foundvertex = (char *) (alignptr + (unsigned long) vertices.getAlignBytes()
                       - (alignptr % (unsigned long) vertices.getAlignBytes()));
...
/*                                                                           */
/*****************************************************************************/
void Mesh::triangledeinit(Behavior *b) {
...
/*                                                                           */
/*****************************************************************************/
void Mesh::maketriangle(Behavior *b, otri *newotri) {
  int i;
...
REAL Mesh::counterclockwise(Behavior *b, vertex pa, vertex pb, vertex pc) {
...
REAL Mesh::incircle(Behavior *b, vertex pa, vertex pb, vertex pc, vertex pd) {
...
/*                                                                           */
/*****************************************************************************/
REAL Mesh::nonregular(Behavior *b, vertex pa, vertex pb, vertex pc, vertex pd) {
  if (b->weighted == 0) {
    return incircle(b, pa, pb, pc, pd);
  } else if (b->weighted == 1) {
    return orient3d(b, pa, pb, pc, pd,
                    pa[0] * pa[0] + pa[1] * pa[1] - pa[2],
                    pb[0] * pb[0] + pb[1] * pb[1] - pb[2],
                    pc[0] * pc[0] + pc[1] * pc[1] - pc[2],
                    pd[0] * pd[0] + pd[1] * pd[1] - pd[2]);
  } else {
    return orient3d(b, pa, pb, pc, pd, pa[2], pb[2], pc[2], pd[2]);
  }
}
...
/*                                                                           */
/*****************************************************************************/
void Mesh::findcircumcenter(Behavior *b,
                            ...
                            vertex circumcenter, REAL *xi, REAL *eta, int offcenter)
{
  ...
  if (b->noexact) {
    ...
    /*   division by zero.                                          */
    denominator = 0.5 / counterclockwise(b, tdest, tapex, torg);
...
#ifndef REDUCED
void Mesh::checkmesh(Behavior *b) {
  otri triangleloop;
  ...
  while (triangleloop.tri != NULL) {
    ...
    for (triangleloop.orient = 0; triangleloop.orient < 3;triangleloop.orient++) {
      ...
      if (triangleloop.orient == 0) {       /* Only test for inversion once. */
        ...
        if (counterclockwise(b, triorg, tridest, triapex) <= 0.0) {
          printf("  !! !! Inverted ");
          printtriangle(b, &triangleloop);
          horrors++;
        }
      }
...
/*                                                                           */
/*****************************************************************************/
void Mesh::checkdelaunay(Behavior *b) {
  otri triangleloop;
  otri oppotri;
  osub opposubseg;
  vertex triorg, tridest, triapex;
  ...
  while (triangleloop.tri != NULL) {
    ...
    for (triangleloop.orient = 0; triangleloop.orient < 3;
      ...
      if (shouldbedelaunay) {
        if (nonregular(b, triorg, tridest, triapex, oppoapex) > 0.0) {
          if (!b->weighted) {
...
/*                                                                           */
/*****************************************************************************/
void Mesh::enqueuebadtriang(Behavior *b, BadTriangle *badtri) {
...
/*                                                                           */
/*****************************************************************************/
void Mesh::enqueuebadtri(Behavior *b, otri *enqtri,
...
#ifndef CDT_ONLY
int Mesh::checkseg4encroach(Behavior *b, osub *testsubseg) {
  otri neighbortri;
  osub testsym;
...
void Mesh::testtriangle(Behavior *b, otri *testtri) {
  otri tri1, tri2;
  osub testsub;
  ...
  if (b->vararea || b->fixedarea || b->usertest) {
    ...
    if (b->fixedarea && (area > b->maxarea)) {
      ...
      enqueuebadtri(b, testtri, minedge, tapex, torg, tdest);
      return;
    }
...
/*                                                                           */
/*****************************************************************************/
void Mesh::makevertexmap(Behavior *b) {
  otri triangleloop;
  vertex triorg;
...
/*                                                                           */
/*****************************************************************************/
LocateResult Mesh::preciselocate(Behavior *b
                                ,vertex searchpoint, otri *searchtri
                                ,int stopatsubsegment)
{
  ...
  while (1) {
    ...
    destorient = counterclockwise(b, forg, fapex, searchpoint);
    /* Does the point lie on the other side of the line defined by the */
    /*   Triangle edge opposite the Triangle's origin?                 */
    orgorient = counterclockwise(b, fapex, fdest, searchpoint);
    if (destorient > 0.0) {
      if (orgorient > 0.0) {
...
/*                                                                           */
/*****************************************************************************/
LocateResult Mesh::locate(Behavior *b, vertex searchpoint, otri *searchtri) {
  VOID **sampleblock;
  char *firsttri;
  otri sampletri;
  vertex torg, tdest;
  ...
  while (SAMPLEFACTOR * samples * samples * samples <
         triangles.getItems()) {
    samples++;
  }
  ...
  while (totalsamplesleft > 0) {
    ...
    firsttri = (char *) (alignptr +
                         (unsigned long) triangles.getAlignBytes() -
                         (alignptr %
                          (unsigned long) triangles.getAlignBytes()));
    ...
    do {
      sampletri.tri = (Triangle *) (firsttri +
                                    (randInt((unsigned int) population) *
                                     triangles.getItemBytes()));
  ...
  ahead = counterclockwise(b, torg, tdest, searchpoint);
  if (ahead < 0.0) {
  ...
  return preciselocate(b, searchpoint, searchtri, 0);
}
...
/*                                                                           */
/*****************************************************************************/
void Mesh::insertsubseg(Behavior *b, otri *tri, int subsegmark) {
  otri oppotri;
  osub newsubseg;
...
/*                                                                           */
/*****************************************************************************/
void Mesh::flip(Behavior *b, otri *flipedge) {
...
/*                                                                           */
/*****************************************************************************/
void Mesh::unflip(Behavior *b, otri *flipedge) {
  otri botleft, botright;
  otri topleft, topright;
  otri top;
  otri botlcasing, botrcasing;
...
/*                                                                           */
/*****************************************************************************/
InserVvertexResult Mesh::insertvertex(Behavior *b
                                     ,vertex    newvertex
                                     ,otri     *searchtri
                                     ,osub     *splitseg
                                     ,int       segmentflaws
                                     ,int       triflaws)
{
  ...
  if (splitseg == NULL) {
    ...
    if (searchtri->tri == dummytri) {
      ...
      intersect = locate(b, newvertex, &horiz);
    } else {
      /* Start searching from the Triangle provided by the caller. */
      otricopy(*searchtri, horiz);
      intersect = preciselocate(b, newvertex, &horiz, 1);
    }
  } else {
  ...
  if ((intersect == ONEDGE) || (intersect == OUTSIDE)) {
    ...
    if (mirrorflag) {
      lnextself(topright);
      sym(topright, toprcasing);
      maketriangle(b, &newtopright);
    } else {
      /* Splitting a boundary edge increases the number of boundary edges. */
      hullsize++;
    }
    maketriangle(b, &newbotright);
    ...
    if (splitseg != NULL) {
      ...
      insertsubseg(b, &newbotright, mark(*splitseg));
    ...
    if (checkquality) {
      ...
      lastflip->prevflip = (FlipStacker *) &insertvertex;
    }

#ifdef SELF_CHECK
    if (counterclockwise(b, rightvertex, leftvertex, botvertex) < 0.0) {
      printf("Internal error in insertvertex():\n");
      printf("  Clockwise Triangle prior to edge vertex insertion (bottom).\n");
    }
    if (mirrorflag) {
      if (counterclockwise(b, leftvertex, rightvertex, topvertex) < 0.0) {
        printf("Internal error in insertvertex():\n");
        printf("  Clockwise Triangle prior to edge vertex insertion (top).\n");
      }
      if (counterclockwise(b, rightvertex, topvertex, newvertex) < 0.0) {
        printf("Internal error in insertvertex():\n");
        printf("  Clockwise Triangle after edge vertex insertion (top right).\n");
      }
      if (counterclockwise(b, topvertex, leftvertex, newvertex) < 0.0) {
        printf("Internal error in insertvertex():\n");
        printf("  Clockwise Triangle after edge vertex insertion (top left).\n");
      }
    }
    if (counterclockwise(b, leftvertex, botvertex, newvertex) < 0.0) {
      printf("Internal error in insertvertex():\n");
      printf("  Clockwise Triangle after edge vertex insertion (bottom left).\n");
    }
    if (counterclockwise(b, botvertex, rightvertex, newvertex) < 0.0) {
      printf("Internal error in insertvertex():\n");
    ...
    if (b->verbose > 2) {
      ...
      printtriangle(b, &botright);
      if (mirrorflag) {
        printf("  Updating top left ");
        printtriangle(b, &topright);
        printf("  Creating top right ");
        printtriangle(b, &newtopright);
      }
      printf("  Creating bottom right ");
      printtriangle(b, &newbotright);
    }
    ...
    if (counterclockwise(b, rightvertex, leftvertex, newvertex) < 0.0) {
      printf("Internal error in insertvertex():\n");
    ...
    if (b->verbose > 2) {
      printf("  Updating top ");
      printtriangle(b, &horiz);
      printf("  Creating left ");
      printtriangle(b, &newbotleft);
    ...
    if (checksegments) {
      ...
      if (checksubseg.ss != dummysub) {
        ...
        if (segmentflaws) {
          ...
          if (checkseg4encroach(b, &checksubseg)) {
    ...
    if (doflip) {
      ...
      if (top.tri == dummytri) {
        ...
        if ((leftvertex == infvertex1) || (leftvertex == infvertex2) ||
          ...
          /*   be applied.                                                */
          doflip = counterclockwise(b, newvertex, rightvertex, farvertex) > 0.0;
          ...
          /*   infinite as well, but trust me, this same condition should */
          /*   be applied.                                                */
          doflip = counterclockwise(b, farvertex, leftvertex, newvertex) > 0.0;
          ...
          doflip = incircle(b, leftvertex, newvertex, rightvertex, farvertex) > 0.0;
        }
        if (doflip) {
...
#ifdef SELF_CHECK
          if (newvertex != NULL) {
            if (counterclockwise(b, leftvertex, newvertex, rightvertex) < 0.0) {
              printf("Internal error in insertvertex():\n");
            ...
            /*   removes.                                                    */
/*
            if (counterclockwise(b, rightvertex, farvertex, leftvertex) < 0.0) {
              printf("Internal error in insertvertex():\n");
              printf("  Clockwise Triangle prior to edge flip (top).\n");
            }
*/
            if (counterclockwise(b, farvertex, leftvertex, newvertex) < 0.0) {
              printf("Internal error in insertvertex():\n");
              printf("  Clockwise Triangle after edge flip (left).\n");
            }
            if (counterclockwise(b, newvertex, rightvertex, farvertex) < 0.0) {
              printf("Internal error in insertvertex():\n");
...
/*                                                                           */
/*****************************************************************************/
void Mesh::triangulatepolygon(Behavior *b
                             ,otri     *firstedge
                             ,otri     *lastedge
                             ,int       edgecount
                             ,int       doflip
                             ,int       triflaws)
{
  ...
  for (i = 2; i <= edgecount - 2; i++) {
    ...
    if (incircle(b, leftbasevertex, rightbasevertex, bestvertex,
                 testvertex) > 0.0) {
  ...
  if (bestnumber > 1) {
    ...
    triangulatepolygon(b, firstedge, &tempedge, bestnumber + 1, 1,
                       triflaws);
  }
  if (bestnumber < edgecount - 2) {
    /* Recursively triangulate the smaller polygon on the left. */
    sym(besttri, tempedge);
    triangulatepolygon(b, &besttri, lastedge, edgecount - bestnumber, 1,
                       triflaws);
...
/*                                                                           */
/*****************************************************************************/
void Mesh::deletevertex(Behavior *b, otri *deltri) {
  otri countingtri;
  ...
  if (edgecount > 3) {
    ...
    triangulatepolygon(b, &firstedge, &lastedge, edgecount, 0, !b->nobisect);
  }
...
/*                                                                           */
/*****************************************************************************/
void Mesh::undovertex(Behavior *b) {
  otri fliptri;
  ...
  while (lastflip != NULL) {
    ...
    } else if (lastflip->prevflip == (FlipStacker *) &insertvertex) {
...
/*                                                                           */
/*****************************************************************************/
void Mesh::mergehulls(Behavior *b
                     ,otri     *farleft
                     ,otri     *innerleft
                     ,otri     *innerright
                     ,otri     *farright
                     ,int       axis)
{
  ...
  do {
    ...
    if (counterclockwise(b, innerleftdest, innerleftapex, innerrightorg) > 0.0) {
      lprevself(*innerleft);
      symself(*innerleft);
    ...
    if (counterclockwise(b, innerrightapex, innerrightorg, innerleftdest) > 0.0) {
      lnextself(*innerright);
      symself(*innerright);
      innerrightorg = innerrightapex;
      apex(*innerright, innerrightapex);
  ...
  while (1) {
    ...
    leftfinished = counterclockwise(b, upperleft, lowerleft, lowerright) <= 0.0;
    rightfinished = counterclockwise(b, upperright, lowerleft, lowerright) <= 0.0;
    if (leftfinished && rightfinished) {
      /* Create the top new bounding Triangle. */
      maketriangle(b, &nextedge);
    ...
    if (!leftfinished) {
      ...
      if (nextapex != NULL) {
        ...
        badedge = incircle(b, lowerleft, lowerright, upperleft, nextapex) > 0.0;
        while (badedge) {
    ...
    if (!rightfinished) {
      ...
      if (nextapex != NULL) {
        /* Check whether the edge is Delaunay. */
        badedge = incircle(b, lowerleft, lowerright, upperright, nextapex) > 0.0;
        while (badedge) {
          /* Eliminate the edge with an edge flip.  As a result, the     */
    ...
    if (leftfinished || (!rightfinished &&
           (incircle(b, upperleft, lowerleft, lowerright, upperright) > 0.0))) {
...
/*                                                                           */
/*****************************************************************************/
void Mesh::divconqrecurse(Behavior *b
                         ,vertex   *sortarray
                         ,int       vertices
                         ,int       axis
                         ,otri     *farleft
                         ,otri     *farright)
{
  ...
  if (vertices == 2) {
    ...
    maketriangle(b, farleft);
    setorg(*farleft, sortarray[0]);
    setdest(*farleft, sortarray[1]);
    /* The apex is intentionally left NULL. */
    maketriangle(b, farright);
    ...
    if (b->verbose > 2) {
      printf("  Creating ");
      printtriangle(b, farleft);
      printf("  Creating ");
      printtriangle(b, farright);
    }
    ...
    maketriangle(b, &midtri);
    maketriangle(b, &tri1);
    maketriangle(b, &tri2);
    maketriangle(b, &tri3);
    area = counterclockwise(b, sortarray[0], sortarray[1], sortarray[2]);
    if (area == 0.0) {
    ...
    if (b->verbose > 2) {
      printf("  Creating ");
      printtriangle(b, &midtri);
      printf("  Creating ");
      printtriangle(b, &tri1);
      printf("  Creating ");
      printtriangle(b, &tri2);
      printf("  Creating ");
      printtriangle(b, &tri3);
    }
    return;
  } else {
    ...
    divconqrecurse(b, sortarray, divider, 1 - axis, farleft, &innerleft);
    divconqrecurse(b, &sortarray[divider], vertices - divider, 1 - axis, &innerright, farright);
    if (b->verbose > 1) {
      printf("  Joining triangulations with %d and %d vertices.\n", divider,
             vertices - divider);
    }
    /* Merge the two triangulations into one. */
    mergehulls(b, farleft, &innerleft, &innerright, farright, axis);
  }
}

long Mesh::removeghosts(Behavior *b, otri *startghost) {
  otri searchedge;
  otri dissolveedge;
  otri deadtriangle;
  vertex markorg;
...
/*                                                                           */
/*****************************************************************************/
long Mesh::divconqdelaunay(Behavior *b) {
  vertex *sortarray;
  ...
  sortarray = (vertex *) trimalloc(invertices * sizeof(vertex));
...
"Warning:  A duplicate vertex at (%.12g, %.12g) appeared and was ignored.\n",
  ...
  divconqrecurse(b, sortarray, i, 0, &hullleft, &hullright);
  trifree((VOID *) sortarray);

  return removeghosts(b, &hullleft);
}
...
/*                                                                           */
/*****************************************************************************/
void Mesh::boundingbox(Behavior *b) {
...
/*                                                                           */
/*****************************************************************************/
long Mesh::removebox(Behavior *b) {
  otri deadtriangle;
  otri searchedge;
  otri checkedge;
...
/*                                                                           */
/*****************************************************************************/
long Mesh::incrementaldelaunay(Behavior *b) {
  otri starttri;
  vertex vertexloop;
  ...
  while (vertexloop != NULL) {
    ...
    if (insertvertex(b, vertexloop, &starttri, NULL, 0, 0) == DUPLICATEVERTEX) {
      if (!b->quiet) {
...
SplayNode *Mesh::circletopinsert(Behavior *b,
                                 SplayNode *splayroot,
                                 otri *newkey,
                                 vertex pa, vertex pb, vertex pc, REAL topy)
{
  ...
  ccwabc = counterclockwise(b, pa, pb, pc);
  xac = pa[0] - pc[0];
  yac = pa[1] - pc[1];
  xbc = pb[0] - pc[0];
  ybc = pb[1] - pc[1];
...
long Mesh::sweeplinedelaunay(Behavior *b) {
  Event **eventheap;
  Event *events;
  Event *freeevents;
  Event *nextevent;
  ...
  maketriangle(b, &lefttri);
  maketriangle(b, &righttri);
  ...
  while (heapsize > 0) {
    ...
    if (nextevent->xkey < xmin) {
      ...
      if (randInt(SAMPLERATE) == 0) {
        ...
        splayroot = circletopinsert(b, splayroot, &lefttri, leftvertex, midvertex, rightvertex, nextevent->ykey);
      }
    } else {
...
*/
    ...
    if (check4events) {
      ...
      lefttest = counterclockwise(b, leftvertex, midvertex, rightvertex);
      if (lefttest > 0.0) {
        newevent = freeevents;
      ...
      righttest = counterclockwise(b, leftvertex, midvertex, rightvertex);
      if (righttest > 0.0) {
        newevent = freeevents;
        freeevents = (Event *) freeevents->eventptr;
        newevent->xkey = xminextreme;
  ...
  return removeghosts(b, &bottommost);
}
...
/*                                                                           */
/*****************************************************************************/
long Mesh::delaunay(Behavior *b) {
  long hulledges;

  eextras = 0;
  initializetrisubpools(b);

#ifdef REDUCED
  if (!b->quiet) {
    printf("Constructing Delaunay triangulation by divide-and-conquer method.\n");
  }
  hulledges = divconqdelaunay(b);
#else /* not REDUCED */
  if (!b->quiet) {
  ...
  if (b->incremental) {
    hulledges = incrementaldelaunay(b);
  } else if (b->sweepline) {
    hulledges = sweeplinedelaunay(b);
  } else {
    hulledges = divconqdelaunay(b);
  }
#endif /* not REDUCED */
...
/*****************************************************************************/
#ifdef TRILIBRARY
int Mesh::reconstruct(Behavior *b
                     ,int      *trianglelist
                     ,REAL     *triangleattriblist
                     ,REAL     *trianglearealist
                     ,int       elements
                     ...
                     ,int       numberofsegments)
#else /* not TRILIBRARY */
long Mesh::reconstruct(Behavior *b
                      ,char     *elefilename
                      ,char     *areafilename
                      ,char     *polyfilename
                      ,FILE     *polyfile)
#endif /* not TRILIBRARY */
{
  ...
  vertexarray = (Triangle *) trimalloc(vertices.getItems() * (int) sizeof(Triangle));
  ...
  while (triangleloop.tri != NULL) {
    ...
    for (j = 0; j < 3; j++) {
      ...
      if ((corner[j] < b->firstnumber) ||
          (corner[j] >= b->firstnumber + invertices)) {
    ...
    for (j = 0; j < 3; j++) {
      ...
      if (*stringptr == '\0') {
        ...
        if ((corner[j] < b->firstnumber) || (corner[j] >= b->firstnumber + invertices)) {
          printf("Error:  Triangle %ld has an invalid vertex index.\n", elementnumber);
          triexit(1);
        }
      }
    ...
    for (j = 3; j < incorners; j++) {
      ...
      if (*stringptr != '\0') {
        ...
        if ((killvertexindex >= b->firstnumber) &&
            (killvertexindex < b->firstnumber + invertices)) {
          /* Delete the non-corner vertex if it's not already deleted. */
          killvertex = getvertex(b, killvertexindex);
    ...
    setorg(triangleloop, getvertex(b, corner[0]));
    setdest(triangleloop, getvertex(b, corner[1]));
    setapex(triangleloop, getvertex(b, corner[2]));
    ...
    while (subsegloop.ss != NULL) {
      ...
      for (j = 0; j < 2; j++) {
        if ((end[j] < b->firstnumber) ||
            (end[j] >= b->firstnumber + invertices)) {
      ...
      segmentorg = getvertex(b, end[0]);
      segmentdest = getvertex(b, end[1]);
...
/*                                                                           */
/*****************************************************************************/
FindDirectionResult Mesh::finddirection(Behavior *b, otri *searchtri, vertex searchpoint) {
  otri checktri;
  vertex startvertex;
  vertex leftvertex, rightvertex;
  ...
  leftccw = counterclockwise(b, searchpoint, startvertex, leftvertex);
  leftflag = leftccw > 0.0;
  /* Is `searchpoint' to the right? */
  rightccw = counterclockwise(b, startvertex, searchpoint, rightvertex);
...
/*                                                                           */
/*****************************************************************************/
void Mesh::segmentintersection(Behavior *b, otri *splittri, osub *splitsubseg, vertex endpoint2) {
  osub opposubseg;
  vertex endpoint1;
  vertex torg, tdest;
  vertex leftvertex, rightvertex;
  ...
  success = insertvertex(b, newvertex, splittri, splitsubseg, 0, 0);
  ...
  collinear = finddirection(b, splittri, endpoint1);
...
/*                                                                           */
/*****************************************************************************/
int Mesh::scoutsegment(Behavior *b, otri *searchtri, vertex endpoint2, int newmark) {
  otri crosstri;
  osub crosssubseg;
  vertex leftvertex, rightvertex;
  FindDirectionResult collinear;
  ...
  collinear = finddirection(b, searchtri, endpoint2);
  dest(*searchtri, rightvertex);
  apex(*searchtri, leftvertex);
  if (((leftvertex[0] == endpoint2[0]) && (leftvertex[1] == endpoint2[1])) ||
    ...
    insertsubseg(b, searchtri, newmark);
    return 1;
    ...
    return scoutsegment(b, searchtri, endpoint2, newmark);
    ...
    if (crosssubseg.ss == dummysub) {
      ...
      segmentintersection(b, &crosstri, &crosssubseg, endpoint2);
...
/*                                                                           */
/*****************************************************************************/
void Mesh::conformingedge(Behavior *b, vertex endpoint1, vertex endpoint2, int newmark) {
  ...
  success = insertvertex(b, newvertex, &searchtri1, NULL, 0, 0);
  if (success == DUPLICATEVERTEX) {
    ...
    if (success == VIOLATINGVERTEX) {
      ...
      success = insertvertex(b, newvertex, &searchtri1, &brokensubseg, 0, 0);
      if (success != SUCCESSFULVERTEX) {
  ...
  /*   segment.                                                             */
  finddirection(b, &searchtri2, endpoint2);
  if (!scoutsegment(b, &searchtri1, endpoint1, newmark)) {
    /* The origin of searchtri1 may have changed if a collision with an */
    /*   intervening vertex on the segment occurred.                    */
    org(searchtri1, midvertex1);
    conformingedge(b, midvertex1, endpoint1, newmark);
  }
  if (!scoutsegment(b, &searchtri2, endpoint2, newmark)) {
    /* The origin of searchtri2 may have changed if a collision with an */
    /*   intervening vertex on the segment occurred.                    */
    org(searchtri2, midvertex2);
    conformingedge(b, midvertex2, endpoint2, newmark);
  }
}
...
/*                                                                           */
/*****************************************************************************/
void Mesh::delaunayfixup(Behavior *b, otri *fixuptri, int leftside) {
  otri neartri;
  otri fartri;
  osub faredge;
  ...
  if (leftside) {
    if (counterclockwise(b, nearvertex, leftvertex, farvertex) <= 0.0) {
      /* leftvertex is a reflex vertex too.  Nothing can */
      /*   be done until a convex section is found.      */
      return;
    }
  } else {
    if (counterclockwise(b, farvertex, rightvertex, nearvertex) <= 0.0) {
      /* rightvertex is a reflex vertex too.  Nothing can */
      /*   be done until a convex section is found.       */
      return;
    }
  }
  if (counterclockwise(b, rightvertex, leftvertex, farvertex) > 0.0) {
    /* fartri is not an inverted Triangle, and farvertex is not a reflex */
    /*   vertex.  As there are no reflex vertices, fixuptri isn't an     */
    /*   inverted Triangle, either.  Hence, test the edge between the    */
    /*   triangles to ensure it is locally Delaunay.                     */
    if (incircle(b, leftvertex, farvertex, rightvertex, nearvertex) <= 0.0) {
      return;
    }
  ...
  delaunayfixup(b, fixuptri, leftside);
  delaunayfixup(b, &fartri, leftside);
}
...
/*                                                                           */
/*****************************************************************************/
void Mesh::constrainededge(Behavior *b, otri *starttri, vertex endpoint2, int newmark) {
  ...
  do {
    ...
    if ((farvertex[0] == endpoint2[0]) && (farvertex[1] == endpoint2[1])) {
      ...
      delaunayfixup(b, &fixuptri, 0);
      delaunayfixup(b, &fixuptri2, 1);
      done = 1;
    } else {
      /* Check whether farvertex is to the left or right of the segment */
      /*   being inserted, to decide which edge of fixuptri to dig      */
      /*   through next.                                                */
      area = counterclockwise(b, endpoint1, endpoint2, farvertex);
      if (area == 0.0) {
        ...
        if (crosssubseg.ss == dummysub) {
          flip(b, &fixuptri);    /* May create inverted Triangle at left. */
        } else {
          /* We've collided with a segment between endpoint1 and endpoint2. */
          collision = 1;
          /* Insert a vertex at the intersection. */
          segmentintersection(b, &fixuptri, &crosssubseg, endpoint2);
          done = 1;
        }
      }
    }
  ...
  insertsubseg(b, &fixuptri, newmark);
  /* If there was a collision with an interceding vertex, install another */
  /*   segment connecting that vertex with endpoint2.                     */
  if (collision) {
    /* Insert the remainder of the segment. */
    if (!scoutsegment(b, &fixuptri, endpoint2, newmark)) {
      constrainededge(b, &fixuptri, endpoint2, newmark);
    }
  }
}
...
/*                                                                           */
/*****************************************************************************/
void Mesh::insertsegment(Behavior *b, vertex endpoint1, vertex endpoint2, int newmark) {
  otri searchtri1, searchtri2;
  Triangle encodedtri;
  vertex checkvertex;
  Triangle ptr;                         /* Temporary variable used by sym(). */
  ...
  if (checkvertex != endpoint1) {
    ...
    if (locate(b, endpoint1, &searchtri1) != ONVERTEX) {
  ...
  /*   toward the second.                                   */
  if (scoutsegment(b, &searchtri1, endpoint2, newmark)) {
  ...
  if (checkvertex != endpoint2) {
    ...
    if (locate(b, endpoint2, &searchtri2) != ONVERTEX) {
      printf("Internal error in insertsegment():  Unable to locate PSLG vertex\n");
  ...
  /*   toward the first.                                     */
  if (scoutsegment(b, &searchtri2, endpoint1, newmark)) {
    /* The segment was easily inserted. */
    return;
  }
  ...
  if (b->splitseg) {
    ...
    conformingedge(b, endpoint1, endpoint2, newmark);
  } else {
#endif /* not CDT_ONLY */
#endif /* not REDUCED */
    /* Insert the segment directly into the triangulation. */
    constrainededge(b, &searchtri1, endpoint2, newmark);
#ifndef REDUCED
#ifndef CDT_ONLY
  }
#endif /* not CDT_ONLY */
...
/*****************************************************************************/
#ifdef TRILIBRARY
void Mesh::formskeleton(Behavior *b
                       ,int      *segmentlist
                       ,int      *segmentmarkerlist
                       ,int       numberofsegments)
#else /* not TRILIBRARY */
...
#endif /* not TRILIBRARY */
      if ((end1 < b->firstnumber) || (end1 >= b->firstnumber + invertices)) {
        if (!b->quiet) {
          printf("Warning:  Invalid first endpoint of segment %d in %s.\n", b->firstnumber + i, polyfilename);
        }
      } else if ((end2 < b->firstnumber) || (end2 >= b->firstnumber + invertices)) {
        if (!b->quiet) {
        ...
        endpoint1 = getvertex(b, end1);
        endpoint2 = getvertex(b, end2);
        if ((endpoint1[0] == endpoint2[0]) && (endpoint1[1] == endpoint2[1])) {
          ...
          insertsegment(b, endpoint1, endpoint2, boundmarker);
        }
      }
    }
  } else {
...
/*                                                                           */
/*****************************************************************************/
void Mesh::infecthull(Behavior *b) {
  otri hulltri;
  otri nexttri;
  otri starttri;
  osub hullsubseg;
...
/*                                                                           */
/*****************************************************************************/
void Mesh::regionplague(Behavior *b, REAL attribute, REAL area) {
  otri testtri;
  otri neighbor;
  Triangle **virusloop;
  Triangle **regiontri;
...
/*                                                                           */
/*****************************************************************************/
void Mesh::carveholes(Behavior *b, REAL *holelist, int holes, REAL *regionlist, int regions) {
  otri searchtri;
  otri triangleloop;
  otri *regiontris;
  Triangle **holetri;
  ...
  if (regions > 0) {
    ...
    regiontris = (otri *) trimalloc(regions * sizeof(otri));
  } else {
    regiontris = NULL;
  }
  ...
  if ((holes > 0) && !b->noholes) {
    ...
    for (i = 0; i < 2 * holes; i += 2) {
      ...
      if ((holelist[i] >= xmin) && (holelist[i] <= xmax)
        ...
        if (counterclockwise(b, searchorg, searchdest, &holelist[i]) > 0.0) {
          /* Find a Triangle that contains the hole. */
          intersect = locate(b, &holelist[i], &searchtri);
    ...
    for (i = 0; i < regions; i++) {
      ...
      if ((regionlist[4 * i] >= xmin)     && (regionlist[4 * i]     <= xmax)
       && (regionlist[4 * i + 1] >= ymin) && (regionlist[4 * i + 1] <= ymax)) {
        ...
        org(searchtri, searchorg);
        dest(searchtri, searchdest);
        if (counterclockwise(b, searchorg, searchdest, &regionlist[4 * i]) > 0.0) {
          /* Find a Triangle that contains the region point. */
          intersect = locate(b, &regionlist[4 * i], &searchtri);
          if ((intersect != OUTSIDE) && (!infected(searchtri))) {
      ...
      if (regiontris[i].tri != dummytri) {
        ...
        if (!deadtri(regiontris[i].tri)) {
          ...
          *regiontri = regiontris[i].tri;
          ...
          regionplague(b, regionlist[4 * i + 2], regionlist[4 * i + 3]);
          /* The virus pool should be empty now. */
        }
      }
    }
...
/*                                                                           */
/*****************************************************************************/
void Mesh::tallyencs(Behavior *b) {
  osub subsegloop;
  int dummy;
  ...
  while (subsegloop.ss != NULL) {
    ...
    dummy = checkseg4encroach(b, &subsegloop);
    subsegloop.ss = subsegtraverse();
  }
}
...
/*                                                                           */
/*****************************************************************************/
void Mesh::splitencsegs(Behavior *b, int triflaws) {
  otri enctri;
  otri testtri;
  osub testsh;
  osub currentenc;
    ...
    while ((encloop != NULL) && (steinerleft != 0)) {
      ...
      if (!deadsubseg(currentenc.ss) &&
        ...
        if (!b->noexact) {
          ...
          multiplier = counterclockwise(b, eorg, edest, newvertex);
        ...
        success = insertvertex(b, newvertex, &enctri, &currentenc, 1, triflaws);
        ...
        dummy = checkseg4encroach(b, &currentenc);
...
/*                                                                           */
/*****************************************************************************/
void Mesh::tallyfaces(Behavior *b) {
  otri triangleloop;
...
/*                                                                           */
/*****************************************************************************/
void Mesh::splittriangle(Behavior *b, BadTriangle *badtri) {
  otri badotri;
  ...
  if (!deadtri(badotri.tri) && (borg == badtri->triangorg) &&
    ...
    newvertex = vertices.alloc();
    findcircumcenter(b, borg, bdest, bapex, newvertex, &xi, &eta, 1);
    ...
    if (((newvertex[0] == borg[0]) && (newvertex[1] == borg[1])) ||
      ...
      success = insertvertex(b, newvertex, &badotri, NULL, 1, 1);
      if (success == SUCCESSFULVERTEX) {
        if (steinerleft > 0) {
          steinerleft--;
        }
...
/*                                                                           */
/*****************************************************************************/
void Mesh::enforcequality(Behavior *b) {
  BadTriangle *badtri;
  int i;
  ...
  tallyencs(b);
  if (b->verbose && (badsubsegs.getItems() > 0)) {
  ...
  if (!b->quiet && b->conformdel && (badsubsegs.getItems() > 0) &&
      (steinerleft == 0)) {
...
/*                                                                           */
/*****************************************************************************/
void Mesh::highorder(Behavior *b) {
...
/*                                                                           */
/*****************************************************************************/
void Mesh::readnodes(Behavior *b, char *nodefilename, char *polyfilename, FILE **polyfile) {
  FILE *infile;
  vertex vertexloop;
  char inputline[INPUTLINESIZE];
  char *stringptr;
...
/*                                                                           */
/*****************************************************************************/
void Mesh::readholes(Behavior *b,
...
/*                                                                           */
/*****************************************************************************/
void Mesh::transfernodes(Behavior *b
                        ,REAL     *pointlist
                        ,REAL     *pointattriblist
                        ,int      *pointmarkerlist
                        ,int       numberofpoints
...
/*****************************************************************************/
#ifdef TRILIBRARY
void Mesh::writenodes(Behavior *b, REAL **pointlist, REAL **pointattriblist, int **pointmarkerlist)
#else /* not TRILIBRARY */
void Mesh::writenodes(Behavior *b, char *nodefilename, int argc, char **argv)
#endif /* not TRILIBRARY */
{
#ifdef TRILIBRARY
  REAL *plist;
  ...
  if ((nextras > 0) && (*pointattriblist == NULL)) {
    *pointattriblist = (REAL *) trimalloc((int) (outvertices * nextras *
                                                 sizeof(REAL)));
  }
  /* Allocate memory for output vertex markers if necessary. */
  if (!b->nobound && (*pointmarkerlist == NULL)) {
...
/*                                                                           */
/*****************************************************************************/
void Mesh::numbernodes(Behavior *b) {
  vertex vertexloop;
  int vertexnumber;
...
/*****************************************************************************/
#ifdef TRILIBRARY
void Mesh::writeelements(Behavior *b
                        ,int     **trianglelist
                        ,REAL    **triangleattriblist)
#else /* not TRILIBRARY */
void Mesh::writeelements(Behavior *b
...
#endif /* not TRILIBRARY */
{
  ...
  if (*trianglelist == NULL) {
    *trianglelist = (int *) trimalloc((int) (triangles.getItems() *
                                             ((b->order + 1) * (b->order + 2) /
                                              2) * sizeof(int)));
  }
  /* Allocate memory for output Triangle attributes if necessary. */
  if ((eextras > 0) && (*triangleattriblist == NULL)) {
    *triangleattriblist = (REAL *) trimalloc((int) (triangles.getItems() * eextras * sizeof(REAL)));
  }
  tlist = *trianglelist;
...
/*****************************************************************************/
#ifdef TRILIBRARY
void Mesh::writepoly(Behavior *b
                    ,int     **segmentlist
                    ,int     **segmentmarkerlist)
#else /* not TRILIBRARY */
void Mesh::writepoly(Behavior *b
...
#endif /* not TRILIBRARY */
{
  ...
  if (*segmentlist == NULL) {
    *segmentlist = (int *) trimalloc((int) (subsegs.getItems() * 2 * sizeof(int)));
  }
  /* Allocate memory for output segment markers if necessary. */
  if (!b->nobound && (*segmentmarkerlist == NULL)) {
    *segmentmarkerlist = (int *) trimalloc((int) (subsegs.getItems() * sizeof(int)));
  }
  slist = *segmentlist;
  ...
  /*   and number of boundary markers (zero or one).                    */
  fprintf(outfile, "%d  %d  %d  %d\n", 0, mesh_dim, nextras, 1 - b->nobound);
  /* Number of segments, number of boundary markers (zero or one). */
  fprintf(outfile, "%ld  %d\n", subsegs.getItems(), 1 - b->nobound);
#endif /* not TRILIBRARY */
...
/*****************************************************************************/
#ifdef TRILIBRARY
void Mesh::writeedges(Behavior *b
                     ,int     **edgelist
                     ,int     **edgemarkerlist)
#else /* not TRILIBRARY */
void Mesh::writeedges(Behavior *b
...
#endif /* not TRILIBRARY */
{
  ...
  if (*edgelist == NULL) {
    *edgelist = (int *) trimalloc((int) (edges * 2 * sizeof(int)));
  }
  /* Allocate memory for edge markers if necessary. */
  if (!b->nobound && (*edgemarkerlist == NULL)) {
    *edgemarkerlist = (int *) trimalloc((int) (edges * sizeof(int)));
  }
  elist = *edgelist;
...
#ifdef TRILIBRARY
            emlist[edgenumber - b->firstnumber] = trisym.tri == dummytri;
#else /* not TRILIBRARY */
...
/*****************************************************************************/
#ifdef TRILIBRARY
void Mesh::writevoronoi(Behavior *b
                       ,REAL    **vpointlist
                       ,REAL    **vpointattriblist
                       ,int     **vpointmarkerlist
                       ,int     **vedgelist
...
#endif /* not TRILIBRARY */
{
  ...
  if (*vpointlist == NULL) {
    *vpointlist = (REAL *) trimalloc((int) (triangles.getItems() * 2 * sizeof(REAL)));
  }
  /* Allocate memory for Voronoi vertex attributes if necessary. */
  if (*vpointattriblist == NULL) {
    *vpointattriblist = (REAL *) trimalloc((int) (triangles.getItems() * nextras * sizeof(REAL)));
  }
  *vpointmarkerlist = NULL;
  ...
  while (triangleloop.tri != NULL) {
    ...
    dest(triangleloop, tdest);
    apex(triangleloop, tapex);
    findcircumcenter(b, torg, tdest, tapex, circumcenter, &xi, &eta, 0);
#ifdef TRILIBRARY
    /* X and y coordinates. */
  ...
  if (*vedgelist == NULL) {
    *vedgelist = (int *) trimalloc((int) (edges * 2 * sizeof(int)));
  }
  *vedgemarkerlist = NULL;
  /* Allocate memory for output Voronoi norms if necessary. */
  if (*vnormlist == NULL) {
    *vnormlist = (REAL *) trimalloc((int) (edges * 2 * sizeof(REAL)));
  }
  elist = *vedgelist;
  normlist = *vnormlist;
  coordindex = 0;
...
#ifdef TRILIBRARY
void Mesh::writeneighbors(Behavior *b, int **neighborlist)
#else /* not TRILIBRARY */
void Mesh::writeneighbors(Behavior *b, char *neighborfilename, int argc, char **argv)
#endif /* not TRILIBRARY */
{
#ifdef TRILIBRARY
  int *nlist;
  ...
  if (*neighborlist == NULL) {
    *neighborlist = (int *)trimalloc((int) (triangles.getItems() * 3 * sizeof(int)));
  }
  nlist = *neighborlist;
  index = 0;
#else /* not TRILIBRARY */
...
/*                                                                           */
/*****************************************************************************/
void Mesh::writeoff(Behavior *b, char *offfilename, int argc, char **argv) {
  FILE *outfile;
  otri triangleloop;
  vertex vertexloop;
  vertex p1, p2, p3;
...
/*                                                                           */
/*****************************************************************************/
void Mesh::quality_statistics(Behavior *b) {
  otri triangleloop;
  vertex p[3];
  ...
  while (triangleloop.tri != NULL) {
    ...
    triarea = counterclockwise(b, p[0], p[1], p[2]);
...
/*                                                                           */
/*****************************************************************************/
void Mesh::statistics(Behavior *b) {
  printf("\nStatistics:\n\n");
...
#endif /* not TRILIBRARY */
{
...
#else /* not TRILIBRARY */
  m.readnodes(&b, b.innodefilename, b.inpolyfilename, &polyfile);
#endif /* not TRILIBRARY */
...
#ifdef CDT_ONLY
  m.hullsize = m.delaunay(&b);                /* Triangulate the vertices. */
#else /* not CDT_ONLY */
  if (b.refine) {
    /* Read and reconstruct a mesh. */
#ifdef TRILIBRARY
    m.hullsize = m.reconstruct(&b
                              ,in->trianglelist
...
#ifdef TRILIBRARY
      m.formskeleton(&b, in->segmentlist, in->segmentmarkerlist, in->numberofsegments);
#else /* not TRILIBRARY */
      m.formskeleton(&b, polyfile, b.inpolyfilename);
#endif /* not TRILIBRARY */
    }
  }
  ...
  if (b.poly && (m.triangles.getItems() > 0)) {
    ...
    if (!b.refine) {
      ...
      m.carveholes(&b, holearray, m.holes, regionarray, m.regions);
    }
  } else {
  ...
  if (b.quality && (m.triangles.getItems() > 0)) {
    m.enforcequality(&b);           /* Enforce angle and area constraints. */
  }
#endif /* not CDT_ONLY */
  ...
  if (b.order > 1) {
    m.highorder(&b);       /* Promote elements to higher polynomial order. */
  }
  if (!b.quiet) {
    printf("\n");
  }
  ...
  if (b.nonodewritten || (b.noiterationnum && m.readnodefile)) {
    ...
    m.numbernodes(&b);         /* We must remember to number the vertices. */
  } else {
