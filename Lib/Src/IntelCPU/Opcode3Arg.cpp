  ...
  if(op3.getType() == IMMEDIATEVALUE) {
    ...
    case REGISTER       :
      ...
      case REGISTER       : // reg,reg,imm
        return InstructionBuilder(*this).setRegRegOperands(op1.getRegister(),op2.getRegister()).setImmediateOperand(op3, op1.getSize());
      case MEMORYOPERAND  : // reg,mem,imm
        return InstructionBuilder(*this).setMemoryRegOperands((MemoryOperand&)op2, op1.getRegister()).setImmediateOperand(op3, op1.getSize());
