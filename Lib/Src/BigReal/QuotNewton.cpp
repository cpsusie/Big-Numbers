...
BigReal BigReal::quotNewton(const BigReal &x, const BigReal &y, const BigReal &f, DigitPool *pool) { // static
  ...
  for(int i = n-1; i >= 0; i--) {
    z += prod(z,dif(pool->_1(),prod(y,z,e(NC.c6,a[i], pool),pool), BigReal::_0, pool), e(u,a[i], pool), pool);
  }
  return prod(x,z+ prod(z,pool->_1() - prod(y, z, g*NC.c6, pool), u*g, pool), f*NC.c7, pool);
}
