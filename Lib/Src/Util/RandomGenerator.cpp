...
UINT64 RandomGenerator::next64(UINT bits) {
...
/*
 * return the next pseudorandom, uniformly distributed int value between 0 (inclusive) and n (exclusive)
...
INT64 RandomGenerator::nextInt64(INT64 n) {
  if(n <= 0) {
    throwInvalidArgumentException(__TFUNCTION__, _T("n must be positive"));
  }
...
float RandomGenerator::nextFloat(float low, float high) {
...
double RandomGenerator::nextGaussian(double mean, double s) {
