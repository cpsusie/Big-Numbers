...
typedef struct {
  ...
  const char *m_longDescription;
} WSAErrorMessage;

static const WSAErrorMessage errorMessages[] = {
   WSAEINTR
...
/*
  ...
  ,WSA_OPERATION_ABORTED
    ,"OS dependent. Overlapped operation aborted."
  ...
  ,WSAPROVIDERFAILEDINIT
    ,"OS dependent. Unable to initialize a service provider."
...
String getWSAErrorText(int errorCode) {
  for(int i = 0; i < ARRAYSIZE(errorMessages); i++) {
    const WSAErrorMessage &msg = errorMessages[i];
    if(msg.m_errorCode == errorCode) {
      return msg.m_shortDescription;
    }
  }

  switch(errorCode) {
#define  prcase(code) case WSA##code: return _T(#code);
    ...
    prcase( EPROTOTYPE      );
    prcase( ENOPROTOOPT     );
    prcase( EPROTONOSUPPORT );
    prcase( ESOCKTNOSUPPORT );
    prcase( EOPNOTSUPP      );
    prcase( EPFNOSUPPORT    );
    prcase( EAFNOSUPPORT    );
    prcase( EADDRINUSE      );
    prcase( EADDRNOTAVAIL   );
    prcase( ENETDOWN        );
    prcase( ENETUNREACH     );
    prcase( ENETRESET       );
    prcase( ECONNABORTED    );
    ...
    prcase( EMFILE          );
    prcase( SYSNOTREADY     );
    prcase( VERNOTSUPPORTED );
    prcase( EFAULT          );
    prcase( NOTINITIALISED  );
    prcase( HOST_NOT_FOUND  );
    prcase( TRY_AGAIN       );
    prcase( NO_RECOVERY     );
    prcase( NO_DATA         );
    default: 
