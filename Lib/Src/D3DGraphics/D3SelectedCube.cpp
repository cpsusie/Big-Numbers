...
D3SelectedCube::D3SelectedCube(D3Scene &scene, const D3Cube &cube) : D3SceneObjectSolidBox(scene, cube) {
  setName(_T("SelectedCube"));
}

void D3SelectedCube::setCube(const D3Cube &cube) {
}
// TODO
/*
void D3SelectedCube::setCube(float minX, float minY, float minZ, float maxX, float maxY, float maxZ) {
  D3Scene &scene = getScene();
  m_cube  = D3Cube(minX,minY,minZ, maxX,maxY,maxZ);
