...
ColorMapObject *GifMakeMapObject(int ColorCount, const GifColorType *ColorMap) {
  const unsigned int bitsPerPixel = GifBitSize(ColorCount);
  const unsigned int mapSize      = 1 << bitsPerPixel;
  ColorMapObject    *Object       = (ColorMapObject*)malloc(sizeof(ColorMapObject));

  if(Object == NULL) {
    return NULL;
  }

  Object->Colors = (GifColorType*)calloc(mapSize, sizeof(GifColorType));
  if(Object->Colors == NULL) {
    free(Object);
    return NULL;
  }

  Object->ColorCount   = mapSize;   // Object->ColorCount MUST be a power of 2. See egif_lib.c EGifPutScreenDesc
  Object->BitsPerPixel = bitsPerPixel;
  Object->SortFlag     = false;
  memset(Object->Colors, 0, mapSize * sizeof(GifColorType));

  if(ColorMap != NULL) { // if ColorCount not a power of 2, the rest will just be zero. 
...
ColorMapObject *GifUnionColorMap(const ColorMapObject *ColorIn1, const ColorMapObject *ColorIn2, GifPixelType ColorTransIn2[]) {
  ...
  for(i = 0; i < ColorIn2->ColorCount && CrntSlot <= 256; i++) {
    ...
    } else { /* Color is new - copy it to a new slot: */
  ...
  if(RoundUpTo != ColorUnion->ColorCount) {
    ...
    if(RoundUpTo < ColorUnion->ColorCount) {
      GifColorType *new_map = (GifColorType *)realloc(Map, sizeof(GifColorType) * RoundUpTo);
      if(new_map == NULL) {
        GifFreeMapObject(ColorUnion);
        return NULL;
      }
...
SavedImage *GifMakeSavedImage(GifFileType *GifFile, const SavedImage *CopyFrom) {
  ...
  if(GifFile->SavedImages == NULL) {
    ...
    if(CopyFrom != NULL) {
      const unsigned int pixelCount = CopyFrom->ImageDesc.Height * CopyFrom->ImageDesc.Width;
      memcpy(sp, CopyFrom, sizeof(SavedImage));
      sp->RasterBits         = NULL; // reset pointers
      ...
      if(CopyFrom->ImageDesc.ColorMap != NULL) {
        sp->ImageDesc.ColorMap = GifMakeMapObject(CopyFrom->ImageDesc.ColorMap->ColorCount
      ...
      /* next, the raster */
      sp->RasterBits = (GifPixelType*)malloc(sizeof(GifPixelType) * pixelCount);
      if(sp->RasterBits == NULL) {
        FreeLastSavedImage(GifFile);
        return NULL;
      }
      memcpy(sp->RasterBits, CopyFrom->RasterBits, sizeof(GifPixelType) * pixelCount);

      /* finally, the extension blocks */
      if(CopyFrom->ExtensionBlocks != NULL) {
        sp->ExtensionBlocks = (ExtensionBlock*)malloc(sizeof(ExtensionBlock) * CopyFrom->ExtensionBlockCount);
