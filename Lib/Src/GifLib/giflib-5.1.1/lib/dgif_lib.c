...
/******************************************************************************
 ...
 info record.
******************************************************************************/
GifFileType *DGifOpenFileName(const char *FileName, int *ErrorCode) {
  int FileHandle;
  GifFileType *GifFile;

  if((FileHandle = open(FileName, _O_RDONLY)) == -1) {
    SETERRORCODE(D_GIF_ERR_OPEN_FAILED);
    return NULL;
  }

  GifFile = DGifOpenFileHandle(FileHandle, ErrorCode);
  return GifFile;
}
...
GifFileType *DGifOpenFileHandle(int FileHandle, int *ErrorCode) {
  ...
  if(GifFile == NULL) {
    SETERRORCODE(D_GIF_ERR_NOT_ENOUGH_MEM);
    close(FileHandle);
    return NULL;
  }
  ...
  if(READ(GifFile, (unsigned char *)Buf, GIF_STAMP_LEN) != GIF_STAMP_LEN) {
    SETERRORCODE(D_GIF_ERR_READ_FAILED);
    fclose(f);
    free(Private);
    free(GifFile);
    return NULL;
  ...
  if(strncmp(GIF_STAMP, Buf, GIF_VERSION_POS) != 0) {
    SETERRORCODE(D_GIF_ERR_NOT_GIF_FILE);
    fclose(f);
    free(Private);
    free(GifFile);
    return NULL;
...
******************************************************************************/
GifFileType *DGifOpen(void *userData, InputFunc readFunc, int *ErrorCode) {
  char Buf[GIF_STAMP_LEN + 1];
  GifFileType *GifFile;
  GifFilePrivateType *Private;
  ...
  if(DGifGetScreenDesc(GifFile) == GIF_ERROR) {
    free(Private);
    free(GifFile);
    SETERRORCODE(D_GIF_ERR_NO_SCRN_DSCR);
    return NULL;
  }
...
int DGifGetScreenDesc(GifFileType *GifFile) {
  ...
  if(READ(GifFile, Buf, 3) != 3) {
    ...
    GifFile->SColorMap = NULL;
    RETURNGIFERROR(D_GIF_ERR_READ_FAILED)
  }
  ...
  if(Buf[0] & 0x80) {    /* Do we have global color map? */
    ...
    if(GifFile->SColorMap == NULL) {
      RETURNGIFERROR(D_GIF_ERR_NOT_ENOUGH_MEM);
    }
...
int DGifGetRecordType(GifFileType *GifFile, GifRecordType *Type) {
  ...
  default:
    *Type = UNDEFINED_RECORD_TYPE;
    RETURNGIFERROR(D_GIF_ERR_WRONG_RECORD);
  }
  return GIF_OK;
}
...
int DGifGetImageDesc(GifFileType *GifFile) {
  ...
  if(Buf[0] & 0x80) {
    ...
    for(i = 0; i < GifFile->Image.ColorMap->ColorCount; i++) { /* coverity[check_return] */
      if(READ(GifFile, Buf, 3) != 3) {
        GifFreeMapObject(GifFile->Image.ColorMap);
        GifFile->Image.ColorMap = NULL;
        RETURNGIFERROR(D_GIF_ERR_READ_FAILED);
...
int DGifGetLine(GifFileType *GifFile, GifPixelType *Line, int LineLen) {
  ...
  if((Private->PixelCount -= LineLen) > 0xffff0000UL) {
    RETURNGIFERROR(D_GIF_ERR_DATA_TOO_BIG);
  }
...
int DGifCloseFile(GifFileType *GifFile, int *ErrorCode) {
  ...
  Private = GETPRIVATE(GifFile);

  if(!IS_READABLE(Private)) {
      /* This file was NOT open for reading: */
    SETERRORCODE(D_GIF_ERR_NOT_READABLE);
  ...
  if(Private->File && (fclose(Private->File) != 0)) {
    SETERRORCODE(D_GIF_ERR_CLOSE_FAILED);
    free(GifFile->Private);
    free(GifFile);
    return GIF_ERROR;
  }
...
static int DGifDecompressLine(GifFileType *GifFile, GifPixelType *Line, int LineLen) {
  ...
  while(i < LineLen) {    /* Decode LineLen items. */
    ...
    if(CrntCode == EOFCode) {
      ...
      .  */
      RETURNGIFERROR(D_GIF_ERR_EOF_TOO_SOON);
      ...
      Private->LastCode    = LastCode = NO_SUCH_CODE;
    } else {
      ...
      if(CrntCode < ClearCode) {
        ...
        if(Prefix[CrntCode] == NO_SUCH_CODE) {
          ...
          if(CrntCode == Private->RunningCode - 2) {
            ...
            Suffix[Private->RunningCode - 2] =
               Stack[StackPtr++] = DGifGetPrefixChar(Prefix, CrntCode, ClearCode);
          }
        } else {
          CrntPrefix = CrntCode;
        }
        ...
        if(StackPtr >= LZ_MAX_CODE || CrntPrefix > LZ_MAX_CODE) {
          RETURNGIFERROR(D_GIF_ERR_IMAGE_DEFECT);
        }
...
int DGifGetLZCodes(GifFileType *GifFile, int *Code) {
  ...
  if(*Code == Private->EOFCode) { /* Skip rest of codes (hopefully only NULL terminating block): */
    do {
...
int DGifSlurp(GifFileType *GifFile) {
  ...
  do {
    ...
    case IMAGE_DESC_RECORD_TYPE:
      ...
      sp->RasterBits = (GifPixelType*)malloc(ImageSize * sizeof(GifPixelType));
    ...
    case EXTENSION_RECORD_TYPE:
      ...
      while(ExtData != NULL) {
        ...
        if(ExtData != NULL) { /* Continue the extension block */
