...
static int SubdivColorMap(NewColorMapType *NewColorSubdiv,
  ...
  while(ColorMapSize > *NewColorMapSize) { /* Find candidate for subdivision: */
    MaxSize = -1;
    for(i = 0; i < *NewColorMapSize; i++) {
      for(j = 0; j < 3; j++) {
...
static int SortCmpRtn(const void *Entry1, const void *Entry2) {
  ...
  int c1 = (((entry1->RGB[SortRGBAxis] << 8) + entry1->RGB[(SortRGBAxis+1) % 3]) << 8) + entry1->RGB[(SortRGBAxis+2) % 3];
  int c2 = (((entry2->RGB[SortRGBAxis] << 8) + entry2->RGB[(SortRGBAxis+1) % 3]) << 8) + entry2->RGB[(SortRGBAxis+2) % 3];
