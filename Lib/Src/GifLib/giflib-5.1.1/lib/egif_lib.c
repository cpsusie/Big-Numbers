...
******************************************************************************/
GifFileType *EGifOpenFileName(const char *FileName, const bool TestExistence, int *ErrorCode) {
  int          FileHandle;
  GifFileType *GifFile;

  if(TestExistence) {
    FileHandle = open(FileName, _O_WRONLY | _O_CREAT | _O_EXCL, _S_IREAD | _S_IWRITE);
  } else {
    FileHandle = open(FileName, _O_WRONLY | _O_CREAT | _O_TRUNC, _S_IREAD | _S_IWRITE);
  }

  if(FileHandle == -1) {
    SETERRORCODE(E_GIF_ERR_OPEN_FAILED);
    return NULL;
  }
  GifFile = EGifOpenFileHandle(FileHandle, ErrorCode);
  if(GifFile == NULL) {
    close(FileHandle);
  }
  return GifFile;
...
******************************************************************************/
GifFileType *EGifOpenFileHandle(int FileHandle, int *ErrorCode) {
  GifFileType        *GifFile;
  ...
  if(Private == NULL) {
    free(GifFile);
    SETERRORCODE(E_GIF_ERR_NOT_ENOUGH_MEM);
    return NULL;
  }
...
#ifdef _WIN32
  _setmode(FileHandle, _O_BINARY);       /* Make sure it is in binary mode. */
#endif /* _WIN32 */

  GifFile->Private    = Private;
  Private->FileHandle = FileHandle;
  Private->File       = _fdopen(FileHandle, "wb");         /* Make it into a stream: */
  Private->FileState  = FILE_STATE_WRITE;
  Private->Write      = NULL;           /* No user write routine (MRB) */
  GifFile->UserData   = NULL;           /* No user write handle (MRB) */
  GifFile->Error      = E_GIF_SUCCEEDED;
...
******************************************************************************/
GifFileType *EGifOpen(void *userData, OutputFunc writeFunc, int *ErrorCode) {
  GifFileType        *GifFile;
  GifFilePrivateType *Private;
...
const char *EGifGetGifVersion(GifFileType *GifFile) {
  GifFilePrivateType *Private = GETPRIVATE(GifFile);
  int i, j;
                      ...
                      const ColorMapObject *ColorMap)
{
  ...
  if(Private->FileState & FILE_STATE_SCREEN) { /* If already has screen descriptor - something is wrong! */
    RETURNGIFERROR(E_GIF_ERR_HAS_SCRN_DSCR);
  }
  CHECKISWRITABLE(Private);

  write_version = EGifGetGifVersion(GifFile);

  /* First write the version prefix into the file. */
  if(InternalWrite(GifFile, (unsigned char*)write_version,strlen(write_version)) != strlen(write_version)) {
    RETURNGIFERROR(E_GIF_ERR_WRITE_FAILED);
  }
  ...
  if(ColorMap) {
    ...
    if(GifFile->SColorMap == NULL) {
      RETURNGIFERROR(E_GIF_ERR_NOT_ENOUGH_MEM);
    }
  } else {
  ...
  if(ColorMap != NULL) {                                    /* If we have Global color map - dump it also:    */
    int i;
    for(i = 0; i < ColorMap->ColorCount; i++) {             /* Put the ColorMap out also:                     */
                     ...
                     const ColorMapObject *ColorMap)
{
  ...
  if(Private->FileState & FILE_STATE_IMAGE && Private->PixelCount > 0xffff0000UL) {
    ...
    RETURNGIFERROR(E_GIF_ERR_HAS_IMAG_DSCR);
  }
  CHECKISWRITABLE(Private);
  ...
  if(GifFile->SColorMap == NULL && GifFile->Image.ColorMap == NULL) {
    RETURNGIFERROR(E_GIF_ERR_NO_COLOR_MAP);
  }
...
int EGifPutLine(GifFileType *GifFile, GifPixelType *Line, int LineLen) {
  ...
  if(Private->PixelCount < (unsigned)LineLen) {
    RETURNGIFERROR(E_GIF_ERR_DATA_TOO_BIG);
  }
...
int EGifPutComment(GifFileType *GifFile, const char *Comment) {
  ...
  if(length <= 255) {
    ...
    while(length > 255) { /* Break the comment into 255 byte sub blocks */
    ...
    if(length > 0) { /* Output any partial block and the clear code. */
...
int EGifCloseFile(GifFileType *GifFile, int *ErrorCode) {
  ...
  if(!IS_WRITEABLE(Private)) { /* This file was NOT open for writing: */
    SETERRORCODE(E_GIF_ERR_NOT_WRITEABLE);
    free(GifFile);
    return GIF_ERROR;
  }
  ...
  if(Private) {
    if(Private->HashTable) {
      free(Private->HashTable);
    }
    free(Private);
  }

  if(File && fclose(File) != 0) {
    SETERRORCODE(E_GIF_ERR_CLOSE_FAILED);
    free(GifFile);
    return GIF_ERROR;
  }

  free(GifFile);
  SETERRORCODE(E_GIF_SUCCEEDED);
  return GIF_OK;
}
...
static int EGifSetupCompress(GifFileType *GifFile) {
 ...
 /* Clear hash table and send Clear to make sure the decoder do the same. */
  ...
  if(EGifCompressOutput(GifFile, Private->ClearCode) == GIF_ERROR) {
    RETURNGIFERROR(E_GIF_ERR_DISK_IS_FULL);
  }
  return GIF_OK;
}
...
******************************************************************************/
static int EGifBufferedOutput(GifFileType *GifFile, GifByteType *Buf,int c) {
  if(c == FLUSH_OUTPUT) { /* Flush everything out. */
