...
static void listTags(const MediaCollection &mc, UINT flags) {
  switch(LISTFLAG(flags)) {
  case LIST_ALLTAGS   :
    mc.list(flags);
    break;
  case LIST_MOBILETAGS:
    { MobileMediaCollection(mc).list(flags);
      break;
    }
  default             :
...
static void usage() {
  _ftprintf(stderr,_T("Usage:mp3Convert [-L[a]|-I] [-s] [-rv] [-p[c]] [-m[textfile]|-f[textfile]|files....]\n"
                      "      -L[a] : List tags.\n"
                      "         a  : list All tags.\n"
                      ...
                      "      -m[textfile]: Read textfile, assumed to be genereted with command mp3convert -L -q...\n"
