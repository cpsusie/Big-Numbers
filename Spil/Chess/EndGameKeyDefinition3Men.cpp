...
EndGamePosIndex EndGameKeyDefinition3Men::keyToIndex(EndGameKey key) const {
  if(!key.kingsOnMainDiag1()) {
    return KK_OFFDIAG_3MEN_INDEX(key) + START_RANGE_KK_OFFDIAG_3MEN  - MININDEX;
  }
  if(!key.p2OnMainDiag1()) {
    return KK_ONDIAG_3MEN_INDEX(key)  + START_RANGE_KK_ONDIAG_3MEN   - MININDEX;
  }
  return KKP2_ONDIAG_3MEN_INDEX(key)  + START_RANGE_KKP2_ONDIAG_3MEN - MININDEX;
}

EndGameKey EndGameKeyDefinition3Men::indexToKey(EndGamePosIndex index) const {
  index += MININDEX;
  EndGameKey result;

  if(index < START_RANGE_KK_ONDIAG_3MEN) {          // king(s) off maindiag => p2 anywhere
    index -= START_RANGE_KK_OFFDIAG_3MEN;
    SETPIT(           result, index   );
