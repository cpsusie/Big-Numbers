...
EndGamePosIndex EndGameKeyDefinition5Men2EqualPawns::keyToIndex(EndGameKey key) const {
  ...
  switch(FBOOL2MASK(IS_KINGSIDE, pos3, pos4)) {
...
void EndGameKeyDefinition5Men2EqualPawns::scanPositions(EndGameKeyWithOccupiedPositions &key, int pIndex) const {
  ...
  case 3:
    ...
    break;
  case 4:
    { const UINT pi3 = s_pawnPosToIndex[key.getPosition(3)];                  // p3 always on queen side
...
void EndGameKeyDefinition5Men2EqualPawns::selfCheck(EndGameKeyWithOccupiedPositions &key) const {
  scanPositions(key, 0);
}

bool EndGameKeyDefinition5Men2EqualPawns::keysEqual(EndGameKey key1, EndGameKey key2) const {
