...
private:
  ...
  bool                    m_timerIsRunning[MAXTIMERID+1];
  ChessWatch              m_watch;
  ...
  MoveFinderThread &getMoveFinder(Player player) {
  ...
  bool   notifyMove(         Player player, const MoveBase &m);
  bool   notifyGameChanged(                 const Game     &game);
  bool   notifyGameChanged(  Player player, const Game     &game);
  bool   isRemote(           Player player) const {
    return m_moveFinder[player]->isRemote();
  }
  bool   isRemoteGame() const {
    return isRemote(WHITEPLAYER) || isRemote(BLACKPLAYER);
  }
  bool   isThinking(         Player player) const {
    return m_moveFinder[player]->isBusy();
  }
  bool   isThinking() const {
    return isThinking(WHITEPLAYER) || isThinking(BLACKPLAYER);
  }
  bool   isThinkingForHint() const {
  ...
  bool   isTimerRunning(unsigned int timerId) const;
  void   showHintMessageBox(const ExecutableMove move);
  ...
  Player getComputerPlayer() const {
