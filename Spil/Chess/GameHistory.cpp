...
Move &Game::getMove(unsigned int ply) { // ply must be in range [0..m_stackSize-1]
  if(ply >= m_stackSize) {
...
const Move &Game::getMove(unsigned int ply) const { // ply must be in range [0..m_stackSize-1]
  if(ply >= m_stackSize) {
    throwInvalidArgumentException(_T("getMove"), _T("ply=%d. plyCount=%d"), ply, m_stackSize);
  }
  return m_gameStack[ply];
