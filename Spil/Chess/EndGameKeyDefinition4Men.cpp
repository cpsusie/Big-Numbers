...
EndGamePosIndex EndGameKeyDefinition4Men::keyToIndex(EndGameKey key) const {
  if(!key.kingsOnMainDiag1()) {
    return KK_OFFDIAG_4MEN_INDEX( key) + START_RANGE_KK_OFFDIAG_4MEN   - MININDEX;
  }
  if(!key.p2OnMainDiag1()) {
    return KK_ONDIAG_4MEN_INDEX(  key) + START_RANGE_KK_ONDIAG_4MEN    - MININDEX;
  }
  if(!key.p3OnMainDiag1()) {
    return KKP2_ONDIAG_4MEN_INDEX(key) + START_RANGE_KKP2_ONDIAG_4MEN  - MININDEX;
  }
  return KKP23_ONDIAG_4MEN_INDEX( key) + START_RANGE_KKP23_ONDIAG_4MEN - MININDEX;
}
...
EndGameKey EndGameKeyDefinition4Men::indexToKey(EndGamePosIndex index) const {
  ...
  if(index < START_RANGE_KK_ONDIAG_4MEN) {           // king(s) off maindiag => p2,p3 anywhere
    index -= START_RANGE_KK_OFFDIAG_4MEN;
    SETPIT(              result, index   );
    ...
    index -= START_RANGE_KK_ONDIAG_4MEN;
    SETPIT(              result, index   );
    SETP3_INDEX(         result, index   );
    ...
    index -= START_RANGE_KKP2_ONDIAG_4MEN;
    SETPIT(              result, index   );
    ...
    index -= START_RANGE_KKP23_ONDIAG_4MEN;
    SETPIT(              result, index   );
