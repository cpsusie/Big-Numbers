...
void MateAnimation::animate() {
  ...
  const Point2DP        margin        = (field45Size-fieldSize)/2;
  ...
  getResources().getFieldMarkImage(CHECKEDKING)->paintImage(background, margin);

  for(SigmoidIterator it(0,180,30); it.hasNext();) {
    const double degree = it.next();
    const Size2DS rotatedSize = PixRect::getRotatedSize(fieldSize, degree);
    helper.rop(ORIGIN, field45Size, SRCCOPY, &background, ORIGIN);
    pieceImage->paintImage(helper, Point2DP((field45Size-rotatedSize)/2), 1, degree);
