...
public:
  ...
  int              m_nodesPerSecond;
  EngineInfoLine(const String &line);
};
  ...
  try {
    for(;;) {
      ...
      if(tok.hasNext() && tok.next() == _T("info")) {
        if(m_verbose) {
          const EngineInfoLine infoLine(tok.getRemaining());
          if(infoLine.m_score.length() > 0) {
            verbose(_T("depth:%2d, nodes:%13s, score:%s\n"), infoLine.m_depth, format1000(infoLine.m_nodes).cstr(), infoLine.m_score.cstr());
          }
          if(infoLine.m_pv.length() > 0) {
            updateMessageField(_T("%s"), getBeautifiedVariant(infoLine.m_pv).cstr());
          }
          if(infoLine.m_msg.length() > 0) {
            verbose(_T("%s\n"), infoLine.m_msg.cstr());
          }
        }
      } else {
...
EngineInfoLine::EngineInfoLine(const String &line) {
    ...
    case ENGINE_SCORE   :
      { const String s = tok.next();
        if(s == _T("cp")) {
          ...
          int moves = tok.getInt();
          if(moves > 0) {
            m_score = format(_T("Mate in %d"), moves);
          } else {
            m_score = format(_T("Loose in %d"), -moves);
          }
        } else if(s == _T("lowerbound")) {
          m_score = format(_T(">%s"), tok.next().cstr());
        } else if(s == _T("upperbound")) {
          m_score = format(_T("<%s"), tok.next().cstr());
        }
      }
      break;
