...
public:
  RegexTester(FILE *output = stdout) : m_output(output) {
  }
  void doTest();
};
...
void RegexTester::callRegex(bool ignoreCase, const String &pattern, const String &target, int expected, bool expectException) {
  ...
  try {
    ...
    if(!expectException)  {
      throwException(_T("error:pattern:<%s> return Exception:<%s>. No Exception expected\n"), pattern.cstr(), e.what());
    }
    return;
  }
  if(expectException) {
    throwException(_T("error:pattern:<%s> expected Exception. No Exception returned\n"),pattern.cstr());
  }
  int ret;


#ifdef _DEBUG
  debugLog(_T("\nPattern:<%s>%s\n"), pattern.cstr(), ignoreCase?_T(" Ignore case") : _T(""));
  debugLog(_T("%s\nFastMap:%s\n"), regex.toString().cstr(), regex.fastMapToString().cstr());
#endif
  ...
  if((ret=regex1.search(target,true,-1,&registers)) != expected) {
    throwException(_T("error:pattern:<%s> target:<%s> return:%d expected:%d\n"),pattern.cstr(), target.cstr(), ret,expected);
  }
  if(pattern != savePattern) {
    throwException(_T("pattern has changed: <%s> -> <%s>\n"),savePattern.cstr(), pattern.cstr());
  }
  if(target != saveTarget) {
    throwException(_T("target has changed: <%s> -> <%s>\n"),saveTarget.cstr(), target.cstr());
  }
#ifdef _DEBUG
  debugLog(_T("--------------------\n"));
#endif
...
void RegexTester::callInvalidPattern(const String &pattern) {
  try {
    Regex reg(pattern);
    throwException(_T("Expected exception"));
  } catch(Exception e) {
    // ok
  }
}
...
void RegexTester::doTest() {
  ...
  const static TCHAR *objectToTest = _T("Regex");
  _ftprintf(m_output,_T("Testing %s%s"),objectToTest,spaceString(15-_tcslen(objectToTest),_T('.')).cstr());
...
void testRegex(FILE *output) {
  try {
    RegexTester tester(output);
    tester.doTest();
  } catch(Exception e) {
#ifdef _DEBUG
