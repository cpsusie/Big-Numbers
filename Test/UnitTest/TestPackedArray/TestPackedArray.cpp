...
/*
  ...
  if(a2 != expected) {
    _tprintf(_T("m_a[%d] = %lu (%s) != expected = %lu (%s) = %s | %s\n")
          , index
  ...
  if(pa2 != expected) {
    _tprintf(_T("m_pa.get(%d) = %lu (%s) != expected = %lu (%s) = %s | %s\n")
          , index
...
bool TestClass::checkEqual() const {
  if(m_pa.size() != m_a.size()) {
    _tprintf(_T("\nSize differs. Array.size=%d, PackedArray.size=%lld\n"), m_a.size(), m_pa.size());
    return false;
  }
  ...
  for(int i = 0; i < n; i++) {
    ...
    if(ea != epa) {
      _tprintf(_T("\nArray[%3d]=%10u, PackedArray.get(%3d)=%10u <----- Error. bitsPerItem:%d\n"), i,ea,i,epa, m_pa.getBitsPerItem());
      return false;
    }
  }
...
#ifdef _DEBUG
void TestClass::dump(const TCHAR *format, ...) const {
  va_list argptr;
  va_start(argptr, format);
  const String label = vformat(format, argptr);
  va_end(argptr);
  _tprintf(_T("%sArray: Size:%d\n"), label.cstr(), m_a.size());
...
void testPackedArray() {
  ...
  for(int i = 0; i < testCount; i++) {
    ...
    if(i % 30 == 0) {
      _tprintf(_T("%s %5.0lf%% BitsPerItem=%2d\r"), verboseStr.cstr(), (double)(i+1)*100.0/testCount, bitsPerItem);
    }

    if(!testAddAndSet(bitsPerItem)) {
      ok = false;
    }

    if(!testAddZeroes(bitsPerItem, size, pos, count, true)) {
      ok = false;
    }

    pos   = randInt() % size;
    count = randInt() % (size - pos);
    if(!testRemove(bitsPerItem, size, pos, count, true)) {
      ok = false;
    }
  }
  ...
  for(int bitsPerItem = 1; bitsPerItem < 32; bitsPerItem++) {
    ...
    printf("Testing PackedArray. bitsPerItem=%d\n", bitsPerItem);
  ...
  if(ok) {
    _tprintf(_T("%50s\r%s%s ok!. Timeusage:%.3lf sec\n"), _T(""), verboseStr.cstr(), objectToTest, (getProcessTime()-startTime)/1000000);
  } else {
    _tprintf(_T("\n%s failed!\n"), objectToTest);
  }
}
