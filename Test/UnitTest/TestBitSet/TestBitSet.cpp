...
BitSet genRandomSet(unsigned int capacity, int size) {
  BitSet result(10);
...
static void measureTime() {
  ...
  _tprintf(_T("%.3le msec\n"), msec*1000);
}

static void testSize() {
  _tprintf(_T("  Testing BitSet.size()..."));
...
//      _tprintf(_T("e:%3d. index1:%3d, index2:%3d\n"), e, index1, index2);
      verify(index1 == index2);
    }
  }
  _tprintf(_T("getIndex() ok!\n"));
}
...
/*
        if(count != expected) {
          _tprintf(_T("(from,to):(%3d,%3d). expected:%3d, got:%3d\n"), from, to, expected, count);
          continue;
        }
*/
  ...
  _tprintf(_T("getCount() ok!\n"));
}

#pragma warning(disable : 4244)

static void testAllBitSetIndices(unsigned int capacity) {
  BitSet s(10);
  genRandomSet(s, capacity, capacity*0.37);
  ...
  _tprintf(_T("\nCapacity:%s size:%11s creation-time:%7.3lf\n"), format1000(capacity).cstr(), format1000(s.size()).cstr(), usedTime);
  ...
  for(Iterator<unsigned int> it = s.getIterator(); it.hasNext(); expectedIndex++) {
    ...
    if(expectedIndex % 50000 == 0) {
      _tprintf(_T("e:%s. index %s\r"), format1000(e).cstr(), format1000(index).cstr());
    }
    if(index != expectedIndex) {
      _tprintf(_T("e:%s. Expected:%d, got %d\n"), format1000(e).cstr(), expectedIndex, index);
      pause();
      int index = bi.getIndex(e);
    }
  }
  usedTime = (getThreadTime() - startTime)/1000000;
  _tprintf(_T("\nIteration-time:%7.3lf\n"), usedTime);
  _tprintf(_T("BitSetIndex ok!\n"));
}

void testBitSetIndex() {
...
*/
  _tprintf(_T("  Testing BitSetIndex...."));
/*
  for(;;) {
...
static void testAllBitSetFileIndices(unsigned int capacity) {
  BitSet s(10);
  genRandomSet(s, capacity, capacity*0.37);
...
//    }
  ...
  usedTime = (getThreadTime() - startTime)/1000000;
  _tprintf(_T("\nIteration-time:%7.3lf\n"), usedTime);
  _tprintf(_T("BitSetFileIndex ok!\n"));
}

void testBitFileSetIndex() {
...
*/
  _tprintf(_T("  Testing BitSetFileIndex...."));
/*
  for(;;) {
    const unsigned int capacity = inputInt(_T("Enter capacity:"));
    testAllBitSetIndices(capacity);
...
void testBitSetIndexTimes() {
  _tprintf(_T("Start testTimes\n"));
  const int capacity = 3000000;
  randomize();
  BitSet s(10);
  genRandomSet(s, capacity, capacity / 2);
  BitSetIndex bi(s);
  _tprintf(_T("  Testing time for BitSetIndex.getIndex()\n"));
  double startTime = getThreadTime();
  for(int e = 0; e < capacity;) {
    bi.getIndex(e);
    if((++e & 0x3ffff) == 0) {
      _tprintf(_T("%.2lf%% Time/call:%.4lf msec\r"), 100.0*e/capacity, (getThreadTime()-startTime)/e);
    }
  }
  double biTime = getThreadTime() - startTime;
  _tprintf(_T("BitSetIndexTime:%.3lf sec\n"), biTime / 1000000);

  _tprintf(_T("  Testing time for BitSet.getIndex()\n"));
  startTime = getThreadTime();
  for(int e = 0; e < capacity; e++) {
    const int index = s.getIndex(e);
  }
  double bitSetTime = getThreadTime() - startTime;
  _tprintf(_T("BitSetTime     :%.3lf sec\n"), bitSetTime / 1000000);
}

void testBitSet() {
  const static TCHAR *objectToTest = _T("BitSet");
