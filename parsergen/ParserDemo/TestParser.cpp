...
SyntaxNode::SyntaxNode(const char *symbol, unsigned int childCount, bool terminal, TestParser *parser) {
  m_symbol     = symbol;
...
static String getLegalInput(const ParserTables &tables, unsigned int state) {
  const unsigned int n = tables.getLegalInputCount(state);
  unsigned int *symbols = new unsigned int[n];
...
void TestParser::buildLegalInputArray() {
  ...
  const unsigned int  stateCount = tables.getStateCount();
...
void TestParser::buildReduceActionArray() {
  const ParserTables &tables = getParserTables();
  for(int prod = 0; prod < tables.getProductionCount(); prod++) {
    m_reduceActionStr.add(format("Reduce by (%d) %s -> %s"
                                 ,prod
...
void TestParser::setNewInput(const char *string) {
...
void TestParser::verror(const SourcePosition &pos, const char *format, va_list argptr) {
  m_ok = false;
  m_handler->handleError(pos, format, argptr);
}

void TestParser::vdebug(const char *format, va_list argptr) {
  m_handler->handleDebug(m_scanner->getPos(), format, argptr);
}

void TestParser::userStackShiftSymbol(unsigned int symbol) {
  SyntaxNode *p = new SyntaxNode((char*)m_scanner->getText(), 0, true, this);
  push(p);
}
...
String TestParser::getActionString() const {
  ...
  if(action == _ParserError) {
    ...
    return format("Shift to state %d", action);
