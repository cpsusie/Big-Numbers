...
void Database::create(SysTableDbData &dbdef) {
  ...
  // creating keydefinition for systable TABLE
    keydef.init();
    keydef.addKeyField(SORT_ASCENDING,DBTYPE_TSTRING,SIZEOF(SysTableTableData,m_tableName));   // tableName
  ...
  // creating keydefinition for systable COLUMN
    keydef.init();
    keydef.addKeyField(SORT_ASCENDING,DBTYPE_TSTRING,SIZEOF(SysTableColumnData,m_tableName));  // tableName
  ...
  // creating keydefinition for systable INDEX1
    keydef.init();
    keydef.addKeyField(SORT_ASCENDING,DBTYPE_TSTRING,SIZEOF(SysTableIndexData,m_tableName));   // tableName
    keydef.addKeyField(SORT_ASCENDING,DBTYPE_TSTRING,SIZEOF(SysTableIndexData,m_indexName));   // indexName
    keydef.addAddrField(true);
  ...
  // creating keydefinition for systable USER
    keydef.init();
    keydef.addKeyField(SORT_ASCENDING,DBTYPE_TSTRING,SIZEOF(SysTableUserData,m_username));     // userName
    keydef.addAddrField(true);
    createSystemKeyFile(dbdef, SYSTEM_USERKEY_FNAME, keydef);

  // creating keydefinition for systable CODE
    keydef.init();
    keydef.addKeyField(SORT_ASCENDING,DBTYPE_TSTRING,SIZEOF(SysTableCodeKey,m_filename));      // filename
  ...
  // creating keydefinition for systable STAT
    keydef.init();
    keydef.addKeyField(SORT_ASCENDING,DBTYPE_TSTRING,SIZEOF(SysTableStatData,m_indexName));    // indexName
    keydef.addAddrField(true);
    ...
    db.sysTabColumnInsert( SYSTABLE , 0,_T("TABLENAME")   ,DBTYPE_TSTRING, OFFSETSIZE(SysTableTableData , m_tableName    ),'N');
    db.sysTabColumnInsert( SYSTABLE , 1,_T("FILENAME")    ,DBTYPE_TSTRING, OFFSETSIZE(SysTableTableData , m_fileName     ),'N');
    db.sysTabColumnInsert( SYSTABLE , 2,_T("SEQUENCENO")  ,DBTYPE_ULONG  , OFFSETSIZE(SysTableTableData , m_sequenceNo   ),'N');
    db.sysTabColumnInsert( SYSTABLE , 3,_T("TABLETYPE")   ,DBTYPE_TSTRING, OFFSETSIZE(SysTableTableData , m_tableType    ),'N');
    db.sysTabColumnInsert( SYSTABLE , 4,_T("RECSIZE")     ,DBTYPE_USHORT , OFFSETSIZE(SysTableTableData , m_recSize      ),'N');

    db.sysTabColumnInsert( SYSCOLUMN, 0,_T("TABLENAME")   ,DBTYPE_TSTRING, OFFSETSIZE(SysTableColumnData, m_tableName    ),'N');
    db.sysTabColumnInsert( SYSCOLUMN, 1,_T("COLNO")       ,DBTYPE_USHORT , OFFSETSIZE(SysTableColumnData, m_colNo        ),'N');
    db.sysTabColumnInsert( SYSCOLUMN, 2,_T("COLUMNNAME")  ,DBTYPE_TSTRING, OFFSETSIZE(SysTableColumnData, m_columnName   ),'N');
    db.sysTabColumnInsert( SYSCOLUMN, 3,_T("DBTYPE")      ,DBTYPE_TSTRING, OFFSETSIZE(SysTableColumnData, m_dbType       ),'N');
    db.sysTabColumnInsert( SYSCOLUMN, 4,_T("OFFSET")      ,DBTYPE_ULONG  , OFFSETSIZE(SysTableColumnData, m_offset       ),'N');
    db.sysTabColumnInsert( SYSCOLUMN, 5,_T("LEN")         ,DBTYPE_ULONG  , OFFSETSIZE(SysTableColumnData, m_len          ),'N');
    db.sysTabColumnInsert( SYSCOLUMN, 6,_T("NULLALLOWED") ,DBTYPE_TSTRING, OFFSETSIZE(SysTableColumnData, m_nulls        ),'N');
    db.sysTabColumnInsert( SYSCOLUMN, 7,_T("DEFAULTVALUE"),DBTYPE_TSTRING, OFFSETSIZE(SysTableColumnData, m_defaultValue ),'N');

    db.sysTabColumnInsert( SYSINDEX , 0,_T("TABLENAME")   ,DBTYPE_TSTRING, OFFSETSIZE(SysTableIndexData , m_tableName    ),'N');
    db.sysTabColumnInsert( SYSINDEX , 1,_T("INDEXNAME")   ,DBTYPE_TSTRING, OFFSETSIZE(SysTableIndexData , m_indexName    ),'N');
    db.sysTabColumnInsert( SYSINDEX , 2,_T("FILENAME")    ,DBTYPE_TSTRING, OFFSETSIZE(SysTableIndexData , m_fileName     ),'N');
    db.sysTabColumnInsert( SYSINDEX , 3,_T("INDEXTYPE")   ,DBTYPE_TSTRING, OFFSETSIZE(SysTableIndexData , m_indexType    ),'N');
    db.sysTabColumnInsert( SYSINDEX , 4,_T("COLCOUNT")    ,DBTYPE_USHORT , OFFSETSIZE(SysTableIndexData , m_colCount     ),'N');
    db.sysTabColumnInsert( SYSINDEX , 5,_T("COLUMNS")     ,DBTYPE_TSTRING, OFFSETSIZE(SysTableIndexData , m_columns      ),'N');
    db.sysTabColumnInsert( SYSINDEX , 6,_T("SORTDIR")     ,DBTYPE_TSTRING, OFFSETSIZE(SysTableIndexData , m_ascending    ),'N');

    db.sysTabColumnInsert( SYSUSER  , 0,_T("USERNAME")    ,DBTYPE_TSTRING, OFFSETSIZE(SysTableUserData  , m_username     ),'N');
    db.sysTabColumnInsert( SYSUSER  , 1,_T("PASSWORD")    ,DBTYPE_TSTRING, OFFSETSIZE(SysTableUserData  , m_password     ),'N');

    db.sysTabColumnInsert( SYSSTAT  , 0,_T("INDEXNAME")   ,DBTYPE_TSTRING, OFFSETSIZE(SysTableStatData  , m_indexName    ),'N');
    db.sysTabColumnInsert( SYSSTAT  , 1,_T("TOTALCOUNT")  ,DBTYPE_DOUBLE , OFFSETSIZE(SysTableStatData  , m_totalCount   ),'N');
    db.sysTabColumnInsert( SYSSTAT  , 2,_T("FIELD1")      ,DBTYPE_DOUBLE , OFFSETSIZE(SysTableStatData  , m_field1       ),'N');
    db.sysTabColumnInsert( SYSSTAT  , 3,_T("FIELD2")      ,DBTYPE_DOUBLE , OFFSETSIZE(SysTableStatData  , m_field2       ),'N');
    db.sysTabColumnInsert( SYSSTAT  , 4,_T("FIELD3")      ,DBTYPE_DOUBLE , OFFSETSIZE(SysTableStatData  , m_field3       ),'N');
    db.sysTabColumnInsert( SYSSTAT  , 5,_T("FIELD4")      ,DBTYPE_DOUBLE , OFFSETSIZE(SysTableStatData  , m_field4       ),'N');

    IndexDefinition indexRec;
  // creating index for systable SYSTABLE KEY1
    indexRec.m_tableName    = SYSTABLE;
    indexRec.m_indexName    = _T("TABLEIX1");
  ...
  // creating index for systable SYSTABLE KEY2
    indexRec.m_tableName    = SYSTABLE;
    indexRec.m_indexName    = _T("TABLEIX2");
  ...
  // creating index for systable SYSCOLUMN
    ...
    indexRec.m_indexName    = _T("COLUMNIX");
  ...
  // creating index for systable SYSINDEX KEY1
    ...
    indexRec.m_indexName    = _T("INDEXIX1");
  ...
  // creating index for systable SYSINDEX KEY2
    indexRec.m_tableName    = SYSINDEX;
    indexRec.m_indexName    = _T("INDEXIX2");
  ...
  // creating index for systable SYSUSER
    ...
    indexRec.m_indexName    = _T("USERIX");
  ...
  // creating index for systable SYSCODE
    ...
    indexRec.m_indexName    = _T("CODEIX");
  ...
  // creating index for systable SYSSTAT
    ...
    indexRec.m_indexName    = _T("STATIX");
    ...
    db.sysTabUserInsert( _T("ADM"),_T("ADM")); // create user ADM
    db.trcommit();
  } catch(sqlca ca) {
    if(insertDone) {
