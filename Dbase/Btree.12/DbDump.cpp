...
void Database::dump(FILE *f, bool dumpall) {
  ...
  for(Iterator<Entry<String,TableDefinition> > it1 = m_tableDefTableNameCache.entrySet().getIterator(); it1.hasNext();) {
    Entry<String,TableDefinition> &entry = it1.next();
    _ftprintf(f,_T("tableDefTableNameCache:<%s>\n"),entry.getKey().cstr());
    entry.getValue().dump(f);
  }
  for(Iterator<Entry<long,TableDefinition*> > it2 = m_tableDefSequenceCache.entrySet().getIterator(); it2.hasNext();) {
    Entry<long,TableDefinition*> &entry = it2.next();
    _ftprintf(f,_T("tableDefSequenceCache:<%d>\n"),entry.getKey());
    entry.getValue()->dump(f);
  }

  for(Iterator<Entry<String,IndexArray> > it3 = m_indexTableNameCache.entrySet().getIterator(); it3.hasNext(); ) {
  ...
  for(Iterator<Entry<String,IndexDefinition*> > it4 = m_indexDefIndexNameCache.entrySet().getIterator(); it4.hasNext(); ) {
