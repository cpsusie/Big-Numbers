...
void PipeLine::write(const Tuple &v) {
  ...
  m_count++;
  m_emptysem.signal(); // tell any thread, waiting in remove that we are not empty anymore
  m_gate.signal(); // open gate
}
...
bool PipeLine::read(Tuple &v) {
  ...
  if(m_buffer[m_head].size() > 0) {
    ...
    m_count--;
    m_fullsem.signal(); // tell any thread waiting in insert, that we are not full anymore
    m_gate.signal(); // open gate
    return true;
  }
  else { // eof
