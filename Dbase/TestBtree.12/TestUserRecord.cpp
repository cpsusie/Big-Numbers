...
public:
  ...
  Array<DbAddr> getAddressArray();
  Array<KeyType> getKeyArray(int index);
...
Array<DbAddr> TableMap::getAddressArray() {
...
Array<KeyType> TableMap::getKeyArray(int index) {
...
//    KeyDefinition keydef        = tableIndex.getKeydef();
    ...
    while(it.hasNext()) {
      ...
      if(!equals(dbTuple,mapTuple)) {
        _tprintf(_T("dbTuple :")); dbTuple.dumpTypes();  _tprintf(_T("\n")); dbTuple.dump();  _tprintf(_T("\n")); 
        _tprintf(_T("mapTuple:")); mapTuple.dumpTypes(); _tprintf(_T("\n")); mapTuple.dump(); _tprintf(_T("\n"));
        verify(false);
      }
    }
    verify(!cursor.hasNext());
...
static void createDatabase(TestFields &testFields) {
...
void testRemove(TableMap &tableMap) {
  Array<DbAddr> addressArray = tableMap.getAddressArray();
...
#define LOOPCOUNT 3
  ...
  for(int i = 0; i < LOOPCOUNT; i++) {
    int k;
    for(k = 0; k < 2; k++) {
      _tprintf(_T("testUserRecord %d,%d\r"),i,k);
