...
try {                                                                                                    \
  ...
  if(ca.sqlcode != expectedError) {                                                                      \
    _ftprintf(stderr,_T("Wrong sqlcode received from %s in %s line %d. sqlca.sqlcode=%d. Expected=%d\n") \
...
#define verifyCast(v,dbtype,ctype,expected) { x = v; x.setType(dbtype); ctype cv; x.get(cv); verify(cv == expected); }
...
#define MAX_INT64CAST ((1ui64<<53)-1)
  ...
  verifyThrows(verifyCast(256,DBTYPE_SHORT,unsigned char,256),SQL_RANGEERROR);
  verifyThrows(verifyCast( -1,DBTYPE_SHORT,unsigned char, -1),SQL_RANGEERROR);
  verifyThrows(verifyCast(  1,DBTYPE_DATE ,Date         ,_T("1")),SQL_INVALIDCAST);
...
//    _tprintf(_T("difference:%lf\n"),difference);
  ...
  verifyThrows(verifyCast(1,DBTYPE_TIMESTAMP,Timestamp,_T("0:0:1")),SQL_INVALIDCAST);
}
