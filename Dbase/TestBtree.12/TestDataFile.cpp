...
class TestDataFile : DbFileTester {
private:
  DataFile         *m_dataFile;
  CompactArray<RecordData> m_dataList;
  CompactArray<RecordData> m_backup;
public:
  TestDataFile(bool useTransaction = false);
  ~TestDataFile();
...
TestDataFile::TestDataFile(bool useTransaction) : DbFileTester(useTransaction) {
...
void TestDataFile::insert(BYTE pattern, UINT size) {
  RecordData rd(pattern,size);
  rd.m_addr = m_dataFile->insert(rd.getRecord(), rd.getSize());
  m_dataList.add(rd);
}

void TestDataFile::update(UINT index, BYTE pattern) {
...
void TestDataFile::remove(UINT index) {
  RecordData &rd = m_dataList[index];
  m_dataFile->remove(rd.m_addr);
  m_dataList.remove(index);
}

void TestDataFile::checkData() {
...
void TestDataFile::beginTrans() {
  DbFileTester::beginTrans();
  m_backup = m_dataList;
}

void TestDataFile::commitTrans() {
  DbFileTester::commitTrans();
  m_backup.clear();
}

void TestDataFile::abortTrans() {
  DbFileTester::abortTrans();
  m_dataList = m_backup;
}

UINT TestDataFile::randomRecordSize() const {
...
void testSimpleDataFile() {
  _tprintf(_T("testSimpleDataFile\n"));
...
void testBackLoggedDataFile() {
  _tprintf(_T("testBackLoggedDataFile\n"));

  TestDataFile test(true);
