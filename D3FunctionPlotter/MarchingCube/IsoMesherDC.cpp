#include "stdafx.h"
#include "IsoMesherDC.h"
#include "QEF.h"

using namespace ThreeD;

IsoMesherDC::IsoMesherDC(Isosurface &iso) : IsoMesher(iso) {
}

Mesh *IsoMesherDC::createMesh() {
    // compute unit-aligned bounding box

  BoundingBox bbox = m_iso.getBoundingBox(createIdentityMatrix());
  int xmin = (int)floor(bbox.vmin().x);
  int ymin = (int)floor(bbox.vmin().y);
  int zmin = (int)floor(bbox.vmin().z);
  int xmax = (int)ceil(bbox.vmax().x + 3 * m_voxelSize.x);
  int ymax = (int)ceil(bbox.vmax().y + 3 * m_voxelSize.y);
  int zmax = (int)ceil(bbox.vmax().z + 3 * m_voxelSize.z);

  m_xsize   = (int)((xmax - xmin) / m_voxelSize.x + 1);
  int ysize = (int)((ymax - ymin) / m_voxelSize.y + 1);
  m_zsize   = (int)((zmax - zmin) / m_voxelSize.z + 1);
  m_zsize   = ((m_zsize + 3) >> 2) << 2;  // round to x4

  if(m_xsize < 1 || ysize < 2 || m_zsize < 1) {
    return NULL;
  }
  ...
  D3DXVECTOR3 vRow = D3DXVECTOR3(xmin, ymin, zmin);
  D3DXVECTOR3 deltaRow = D3DXVECTOR3(0, m_voxelSize.y, 0);
  ...
  for(y = 0; y < 3; ++y) {
    rows[y] = &row_space[y];
    rows[y]->points    = new D3DXVECTOR3[m_xsize * m_zsize];
    rows[y]->densities = new float[ m_xsize * m_zsize];
    rows[y]->cubes     = new Cube[  m_xsize * m_zsize];
  ...
  for(y = 0; y < 3; ++y) {
    delete[] rows[y]->points;
    delete[] rows[y]->densities;
    delete[] rows[y]->cubes;
  }
...
bool IsoMesherDC::computePoints(Row *row) {
  float x0 = row->v.x;
  float y0 = row->v.y;
  float z0 = row->v.z;
  float dz = m_voxelSize.z;

  for(int x = 0; x < m_xsize; ++x) {
    float *densities = &row->densities[x * m_zsize];
    m_iso.fDensity(x0, y0, z0, dz, m_zsize, densities);

    D3DXVECTOR3 *points = &row->points[x * m_zsize];
    for(int z = 0; z < m_zsize; ++z) {
      densities[z] += 1e-4f;
      points[z] = D3DXVECTOR3(x0, y0, z0 + z * dz);
    }
    x0 += m_voxelSize.x;
  }
  return false;
...
#define DENSITY_VAL(xi,yi,zi) rows[yi]->densities[(x + xi) * m_zsize + (z + zi)]
#define POINT_PTR(xi,yi,zi) &rows[yi]->points[(x + xi) * m_zsize + (z + zi)]
...
bool IsoMesherDC::computeCubes(Row *rows[2]) {
  int xsize_1 = m_xsize - 1;
  int zsize_1 = m_zsize - 1;

  for(int x = 0; x < xsize_1; ++x) {
    for(int z = 0; z < zsize_1; ++z) {
      Cube *cube = &rows[0]->cubes[x * m_zsize + z];
...
void IsoMesherDC::generateVertex(Cube *cube, Point corners[8]) {
  //
  ...
  int edgeInfo = s_edgeTable[cube->index];

  Point  points[12];
  D3DXVECTOR3 points_v[12];
  D3DXVECTOR3 normals[12];
  D3DXVECTOR3 massPoint(0, 0, 0);
  ...
  for(i = 0; i < 12; ++i) {
    ...
    if(fabs(corners[n1].density) < 1e-3) {
      ...
      D3DXVECTOR3 v_diff = abs(*corners[n1].v - *corners[n2].v);
      if(v_diff.x > 1e-3) {
        intersectXaxis(corners[n1], corners[n2], points[i]);
      } else if(v_diff.y > 1e-3) {
        intersectYaxis(corners[n1], corners[n2], points[i]);
      } else if(v_diff.z > 1e-3) {
        intersectZaxis(corners[n1], corners[n2], points[i]);
      }
    }

    m_iso.fNormal(*points[i].v, normals[i]);
  ...
  for(i = 0; i < 12; ++i) {
    ...
    D3DXVECTOR3 p = *points[i].v - massPoint;
...
bool IsoMesherDC::generateQuads(Row *rows[2]) {
  int xsize_2 = m_xsize - 2;
  int zsize_2 = m_zsize - 2;

#define CUBE_PTR(xi,yi,zi) &rows[yi]->cubes[(x + xi) * m_zsize + (z + zi)]
  ...
  for(int x = 0; x < xsize_2; ++x) {
    for(int z = 0; z < zsize_2; ++z) {
      ...
      int cube0_edgeInfo = s_edgeTable[cubes[0]->index];
      ...
      for(int i = 0; i < 3; ++i) {
        ...
        for(int j = 1; j < 3; ++j) {
          ...
          if(!meshPlane) {
            ...
            meshPlane = m_mesh->addPlane(p);
          }

          m_mesh->addFace(face, meshPlane);
        }
      }
    }
...
void IsoMesherDC::generateFace(MeshPoint *p0, MeshPoint *p1, MeshPoint *p2, int flip_if_nonzero) {
  float dummy;
#define nsign(x) (dummy = x, dummy == 0.0 ? 0 : 1 - 2 * fsign(dummy))
  ...
  D3DXVECTOR3 a = (p0->point - p2->point);
  D3DXVECTOR3 b = (p1->point - p2->point);
  D3DXVECTOR3 n9 = a % b;
...
int IsoMesherDC::s_edgeTable[256] = {
