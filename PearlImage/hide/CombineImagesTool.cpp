...
void CombineImagesTool::OnMouseMove(UINT nFlags, const CPoint &point) {
  if(nFlags & MK_LBUTTON) {
    if(m_copy == NULL) {
      ...
      getPixRect()->rop(m_rect.TopLeft(),m_rect.Size(),SRCINVERT, m_copy, ORIGIN);
    }
    repaint();
  }
}
...
void CombineImagesTool::OnLButtonUp(UINT nFlags, const CPoint &point) {
  if(m_copy == NULL) {
    ...
    if(m_rect.Width() > 0 && m_rect.Height() > 0) {                                              // if valid rectangle
      m_copy = new PixRect(m_rect.Size());                                                       //   define copy
      m_old  = new PixRect(m_rect.Size());                                                       //   define old
      m_copy->rop(ORIGIN,m_rect.Size(),NOTSRCCOPY, getPixRect(),m_rect.TopLeft());               //   take a copy
...
void CombineImagesTool::makeFinalImage() {
  ...
  for(p.y = 0; p.y < height; p.y++) {
    for(p.x = 0; p.x < width; p.x++) {
      if((RGBA_TORGB(mpa->getPixel(p) ^ opa->getPixel(p))) != 0) {
        rpa->setPixel(p,RGBA_SETALPHA(BLACK,0));
      }
    }
  }
  delete rpa;
  ...
  delete m;
  getPixRect()->rop(m_rect.TopLeft(),m_rect.Size(),SRCCOPY,result,ORIGIN);
  delete result;
}
