...
void TextTool::OnLButtonDown(UINT nFlags, const CPoint &point) {
  ...
  if(dlg.DoModal() == IDOK) {
    ...
    PixRectFont font(m_fontParameters.m_logFont,m_fontParameters.m_orientation);
    if(m_bezier) {
      getPixRect()->drawText(point,dlg.m_text,font,m_container->getColor());
    } else {
      getPixRect()->text(point,dlg.m_text,font,m_container->getColor());
    }
    m_container->repaint();
