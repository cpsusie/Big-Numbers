...
void PolygonTool::invertDragLine() {
  getPixRect()->line(m_polygon.last(),m_p0,0,true);
  repaint();
}
...
void PolygonTool::OnLButtonDown(UINT nFlags, const CPoint &point) {
  if(m_polygon.size() == 0) {
    ...
    m_p0 = point;
    getPixRect()->line(m_polygon.last(),m_p0,m_container->getColor());
    m_polygon.add(point);
    invertDragLine(); // draw dragline
  }
  repaint();
...
void PolygonTool::OnLButtonDblClk(UINT nFlags, const CPoint &point) {
  if(m_polygon.size() == 1) {
    ...
    invertDragLine(); // remove dragline
    getPixRect()->line(m_polygon.last(),m_polygon[0],m_container->getColor());
    getPixRect()->fillPolygon(m_polygon,m_container->getColor());
  }
  repaint();
  m_polygon.clear();
}
