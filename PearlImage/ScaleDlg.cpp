...
void CScaleDlg::setLabels() {
  if(IsDlgButtonChecked(IDC_RADIOPERCENT)) {
    GetDlgItem(IDC_STATICXLABEL2)->SetWindowText(_T("%"));
    GetDlgItem(IDC_STATICYLABEL2)->SetWindowText(_T("%"));
  } else {
    GetDlgItem(IDC_STATICXLABEL2)->SetWindowText(_T("pixels"));
    GetDlgItem(IDC_STATICYLABEL2)->SetWindowText(_T("pixels"));
  }
}
