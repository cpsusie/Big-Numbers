...
public:
  ...
  virtual const char *getExpr() const  = 0;
...
ExpressionTest::~ExpressionTest() {
  if(!m_executed) {
    printf("One or more Expression tests has not been not executed\n");
  }
}
...
public:
  const char *getExpr() const {
    return "poly[1,poly[2,-1,3](x),3,4](x)";
  }
  Real fr(const Real &x) const {
    Real c1[3] = { 2,-1,3 };
    Real c2[4] = { 1,0,3,4 };
...
public:
  const char *getExpr() const {
    return "sum(i=0 to 10) x^i/fac(i)";
  }
  Real fr(const Real &x) const {
    Real sum = 0;
    for(int i = 0; i <= 10; i++)
...
public:
  const char *getExpr() const {
    return "product(i=1 to 10) (x+i+0.25)/(x+i+1)";
  }
  Real fr(const Real &x) const {
    Real product = 1;
    for(int i = 1; i <= 10; i++)
  ...
  try {
    for(int i = 0; i < ARRAYSIZE(testCases); i++) {
      ...
      if(!compiledExpr.isOk()) {
        tcout << endl << _T("Error in testcase[") << i << _T("]<") << expr << _T(">") << endl; compiledExpr.listErrors(tcout);
        abort();
      } else {
        for(Real x = -2; x <= 2; x+=0.5) {
          compiledExpr.setValue(   "x", x);
          interpreterExpr.setValue("x", x);
          ...
          case EXPR_RETURN_REAL:
            { const Real compiledResult     = compiledExpr.evaluate();
              ...
              if((compiledDefined != interpreterDefined) || (compiledDefined && fabs(compiledResult - interpreterResult) > 1e-15)) {
                tcout << endl 
                      << _T("TestCase[") << i << _T("]:<") << expr << _T(">(x=") << toString(x)   << _T(") failed.") << endl
    ...
    tcout << endl << _T("Exception:") << e.what() << endl;
    abort();
  }
}

void testExpression() {
  tcout << _T("Testing Expression...") << endl;

  doTestExpression();
//pause();
  tcout << _T("\nExpression ok.") << spaceString(40) << endl;
}
