...
StreamParameters nparam( 8,18, ios::scientific|ios::left|ios::showpos);
StreamParameters iparam( 0,18, ios::fixed     |ios::left|ios::showpos);
StreamParameters i0param(0,0 , ios::fixed);
...
bool PSLQ::solve() {
  ...
  for(;;) {
    ...
    if(m_verbose) {
      tcout << _T("AH:") << endl << nparam << AH;
      tcout << _T("A:")  << endl << iparam << m_A;
    }
...
// Returns Matrix D so D*m is _T("as diagonal as possible")
BigRealMatrix PSLQ::createHermiteReducingMatrix(const BigRealMatrix &m) {
  static const BigReal half = 0.5;
...
//  tcout << _T("y:") << nparam << y << endl;
  ...
  if(m_verbose)
    tcout << _T("min:") << nparam << minimum << _T("  |min/max|:") << nparam << q << endl;
  ...
  for(int degree = 2; degree <= 30; degree++) {
    ...
    if(pslq.solve()) {
      tcout << _T("Found integer polynomial of degree:") << degree << _T(".")
           << _T(" c(0)..c(") << degree << _T("):")
           << _T(":") << i0param << pslq.getSolution() << endl;
...
static void findIntegerRelation(const Array<BigReal> &a, int digits, bool verbose) {
  ...
  PSLQ pslq(x,digits,verbose);
  if(pslq.solve()) {
    tcout << _T("Integer relation:") << i0param << _T(":") << pslq.getSolution() << endl;
    return;
  } else {
...
static void testSuite(int digits, bool verbose) {
  ...
  findIntegerPolynomial(BigReal(_T("3.6502815398728847452")),digits,verbose);
}
      ...
      for(cp++; *cp; cp++) {
        ...
        case 'x':
          { if(cmd != NO_COMMAND)
            ...
            cmd = FIND_RELATION;
            Tokenizer tok(cp+1,",");
            for(char *t = tok.next(); t; t = tok.next()) {
              v.add(BigReal(t));
            }
          }
          break;
