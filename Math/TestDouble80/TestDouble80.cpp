...
static const StreamParameters nameParam(0,16,ios::left);

static double getRelativeError(double x64, const Double80 &x80) {
  double relativeError = fabs(x64 - getDouble(x80));
...
static void testFunction(const String &name, Double80 (*f80)(const Double80 &), double (*f64)(double), double low, double high) {
  ...
  tcout << _T("Testing function ") << nameParam << name;
  ...
  for(double x64 = low; x64 <= high; x64 += step) {
    ...
    Double80 x80 = x64;
    Double80 y80 = f80(x80);

    double relativeError = getRelativeError(y64,y80);

    if(x64 != getDouble(x80) || relativeError > EPS) {
      tcout << NEWLINE;
      tcout << _T("Function ") << name << _T(" failed.") << NEWLINE;
      tcout << _T("(x64,y64):(") << dparam(16) << x64 << _T(",") << dparam(16) << y64 << _T(").") << NEWLINE;
      tcout << _T("x64 - getDouble(x80):") << dparam(16) << (x64 - getDouble(x80)) << _T(".") << NEWLINE;
      tcout << _T("y64 - getdouble(y80):") << dparam(16) << (y64 - getDouble(y80)) << _T(".") << NEWLINE;
      tcout << _T("RelativeError:") << dparam(16) << relativeError << NEWLINE;
      abort();
    }
  ...
  tcout << _T(" Max relative error:") << dparam(16) << maxRelativeError << NEWLINE;
}
...
static void checkResult(double x64, Double80 x80, TCHAR *op) {
  double relativeError = getRelativeError(x64,x80);

  if(relativeError > EPS) {
    _ftprintf(stderr,_T("operator %s failed. x64=%20.16le x80=%s. Relative error:%le \n"),op,x64,toString(x80).cstr(),relativeError);
    abort();
  }
}
...
static void checkResult(double x64, double y64, bool cmp64, Double80 x80, Double80 y80, bool cmp80, TCHAR *relation) {
  if(cmp64 != cmp80) {
    _ftprintf(stderr,_T("Relation %s failed. %20.16le %s %20.16le = %s. %s %s %s = %s.\n")
        , relation
...
static void testFunction(const String &name, Double80 (*f80)(const Double80 &, const Double80 &), double (*f64)(double,double), double low1, double high1, double low2, double high2) {
  ...
  double maxRelativeError = 0;
  double step1 = (high1 - low1) / 10;
  double step2 = (high2 - low2) / 10;
  for(double x64 = low1; x64 <= high1; x64 += step1) {
    for(double y64 = low2; y64 <= high2; y64 += step2) {
      ...
      double relativeError = getRelativeError(z64,z80);

      if(x64 != getDouble(x80) || y64 != getDouble(y80) || relativeError > EPS) {
        tcout << NEWLINE;
        tcout << _T("Function ") << name << _T(" failed.") << NEWLINE;
        tcout << _T("(x64,y64,z64):(") << dparam(16) << x64 << _T(",") << dparam(16) << y64 << _T(",") << dparam(16) << z64 << _T(").") << NEWLINE;
        tcout << _T("x64 - getDouble(x80):") << dparam(16) << (x64 - getDouble(x80)) << _T(".") << NEWLINE;
        tcout << _T("y64 - getdouble(y80):") << dparam(16) << (y64 - getDouble(y80)) << _T(".") << NEWLINE;
        tcout << _T("z64 - getdouble(z80):") << dparam(16) << (z64 - getDouble(z80)) << _T(".") << NEWLINE;
        tcout << _T("RelativeError:") << dparam(16) << relativeError << NEWLINE;
        abort();
      }
      if(relativeError > maxRelativeError)
...
static void testDouble80io() {
  tcout << _T("Testing Double80 <-> String.") << NEWLINE;
  ...
  for(int i = 1; i >= -1; i-=2) {
    for(double d64 = 1e-100*i; fabs(d64) < 1e100; d64 *= 1.1) {
      ...
      if(s64 != s80) {
        _tprintf(_T("d64:%23.15le -> \"%s\"\n"),d64,s64.cstr());
        _tprintf(_T("                           s80:\"%s\" toString(d80,18):\"%s\"\n"), s80.cstr(), toString(d80,18,0,ios::scientific).cstr());
      }
    }
  }

  Double80 pi = Double80::M_PI;

  Double80 diff = pi - Double80("3.14159265358979324");
...
static double testRound(double x64, int dec) {
  Double80 x80 = x64;
  double   y64 = round(x64,dec);
  Double80 y80 = round(x80,dec);

  double relativeError = getRelativeError(y64,y80);

  if(x64 != getDouble(x80) || relativeError > EPS) {
    _tprintf(_T("Function:%s failed.\n"),_T("round"));
    _tprintf(_T("x64 - getDouble(x80):%le\n"),x64 - getDouble(x80));
    _tprintf(_T("y64 - getdouble(y80):%le\n"),y64 - getDouble(y80));
    _tprintf(_T("RelativeError:%le\n"),relativeError);
    abort();
  }
  return relativeError;
}

static void measureExpo10() {
  Double80::enableDebugString(false);
  double startTime = getProcessTime();
  Double80 stepFactor = 1.0012345;
  int count = 0;
  for(Double80 x = Double80::DBL80_MIN; !isNan(x); x *= stepFactor) {
    int d = Double80::getExpo10(x);
    count++;
  }
  double timeUsage = (getProcessTime() - startTime)/1e3/count;

  _tprintf(_T("TimeMeasure on Double80::getExpo10(): count = %d, Time = %.3le msec.\n"),count,timeUsage);
  Double80::enableDebugString(true);
}
    
static void measureToString() {
  Double80::enableDebugString(false);

  double startTime = getProcessTime();
  Double80 stepFactor = 1.012345;
  int count = 0;
  for(Double80 x = Double80::DBL80_MIN; !isNan(x); x *= stepFactor) {
    String s = x.toString();
    count++;
  ...
  _tprintf(_T("TimeMeasure on Double80::toString(): count = %d, Time = %.3le msec.\n"),count,timeUsage);
  Double80::enableDebugString(true);
}

static void testRound() {

  tcout << _T("Testing function ") << nameParam << _T("round");
  ...
  for(int i = 0; i < ARRAYSIZE(testData); i++) {
    ...
    for(int dec = -21; dec < 21; dec++) {
      ...
      for(int j = 0; j < 10; j++, t*=10) {
        double error = testRound(t,dec);
        if(error > maxRelativeError)
          maxRelativeError = error;
      }
    }
  }
  tcout << _T(" Max relative error:") << dparam(16) << maxRelativeError << _T(".") << NEWLINE;
}
...
static void findDouble80Epsilon() {
...
static void findDouble80Max() {
  tcout << _T("DBL80_MAX:") << Double80::DBL80_MAX.toString() << NEWLINE;
  char buffer[10];
  memcpy(buffer,&Double80::DBL80_MAX,sizeof(buffer));
  hexdump(buffer,sizeof(buffer),stdout);
}

static void testReadWriteDouble80() {
  tcout << _T("Testing read/write Double80") << NEWLINE;
  ...
  for(i = 0; i < list.size(); i++) {
    ...
    in >> x;
    if(in.bad()) {
      tcout << _T("Read Double80 line ") << i << _T(" failed") << NEWLINE;
      throwException(_T("Error in testReadWriteDouble80"));
    }
    const Double80 relError = getRelativeError(x, expected);
    if(relError > maxTolerance ) {
      tcout << _T("Read Double80 at line ") << i << _T("(=") << param << x << _T(") != exptected (=") << param << expected << _T(")") << NEWLINE;
      tcout << _T("Relative error:") << relError << NEWLINE;
      throwException(_T("Error in testReadWriteDouble80"));
    }
  }
  in.close();
...
//  return;
  ...
  tcout << _T("Testing Double80.") << NEWLINE;

  testReadWriteDouble80();
