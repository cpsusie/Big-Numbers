...
namespace IJK {
  ...
  protected:
    ...
    const ERROR & Copy(const ERROR & error)  // copy
    {
      if (&error != this) {
	for (int i = 0; i < error.NumMessages(); i++)
	  { msg[i] = error.Message(i); };
      }
    }
  ...
  public:
    ...
    ERROR(const ERROR & error) { Copy(error); };    // copy constructor
    ...
    void AddMessage(const std::ostringstream & error_msg)     
    { AddMessage(error_msg.str()); };

    template <class T1, class T2, class T3>
    void AddMessage(T1 a1, T2 a2, T3 a3)
    { 
    ...
    void AddMessage(T1 a1, T2 a2, T3 a3, T4 a4, T5 a5)
    {
      std::ostringstream os0;
      os0 << a1 << a2 << a3 << a4 << a5;
      AddMessage(os0);
    ...
    template <class T1, class T2, class T3, class T4, class T5,
	      class T6, class T7>
    void AddMessage(T1 a1, T2 a2, T3 a3, T4 a4, T5 a5, T6 a6, T7 a7)
    {
      std::ostringstream os0;
    ...
    { AddMessage(ProcMessage(procname)); };
    void SetMessage(const int i, const std::string & s)
    { if (i >= 0 && i < NumMessages()) { msg[i] = s; }; };
