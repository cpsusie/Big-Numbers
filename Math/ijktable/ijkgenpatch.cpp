...
void add_simplex_to_isosurface(uint                                      dimension
                              ,const fixedarray<uint>                   &simplex_vert
                              ,uint                                      is
                              ,const fixedarray<uint>                   &point_list
                              ,vector<ISOSURFACE_VERTEX_INDEX>          &isov_list);
                           ...
                           ,long                              &vertex_bits)
{
  if(ip < num_vert_in_point_list) {
    ...
    // point_list[ip] is a polyhedron vertex
    const uint  ie = point_list[ip];

    uint  iv0 = polyhedron.EdgeEndpoint(ie, 0);
    uint  iv1 = polyhedron.EdgeEndpoint(ie, 1);
...
//      (edge_list[d*i], edge_list[d*i+1], ..., edge_list[d*i+d-1])
  uint               nume         = polyhedron.NumEdges();
  uint               numv         = polyhedron.NumVertices();
  uint               dimension    = polyhedron.Dimension();
  fixedarray<uint>   temp_plist(  nume+numv);
...
{
  const uint          dimension   = polyhedron.Dimension();
  fixedarray<uint>   &point_list  = temp_plist;
  ...
  uint  nump = 0;
  for(uint  jv = 0; jv < polyhedron.NumVertices(); jv++) {
    if(vertex_sign[jv] >= 0) {
      // vertex jv is zero or positive
      for(uint  ic = 0; ic < dimension; ic++) {
        uint  j = nump*dimension+ic;
  ...
  uint  num_pos_vertices = nump;

  // add midpoints of bipolar edges to point_list[]
  for(uint  ie = 0; ie < polyhedron.NumEdges(); ie++) {
    uint  iv0 = polyhedron.EdgeEndpoint(ie, 0);
    uint  iv1 = polyhedron.EdgeEndpoint(ie, 1);
  ...
  uint  convex_hull_dimension = 0;
  fixedarray<uint>  simplex_vert(0);
  uint  num_hull_simplices    = 0;
  ...
  for(uint  is = 0; is < num_hull_simplices; is++) {
...
{
  const uint                          dimension   = polyhedron.Dimension();
  const uint                          poly_numv   = polyhedron.NumVertices();
  const uint                          poly_nume   = polyhedron.NumEdges();
  fixedarray<uint>                   &point_list  = temp_plist;
  ...
  uint  nump = 0;
  for(uint  jv = 0; jv < poly_numv; jv++) {
    if(vertex_sign[jv] >= 0) {
      // vertex jv is zero or positive
      for(uint  ic = 0; ic < dimension; ic++) {
        const uint  j = nump*dimension+ic;
        point_coord[j] = polyhedron.VertexCoord(jv, ic);
      }
  ...
  uint  num_vert_in_point_list = nump;
  ...
  if(convex_hull_dimension == dimension) {
    for(uint  is = 0; is < num_hull_simplices; is++) {
      ...
      long vertex_bits = 0L;
      for(uint  ic = 0; ic < dimension; ic++) {
        const uint  ip = simplex_vert[is*dimension+ic];
...
void add_simplex_to_isosurface(uint                             dimension
  ...
  for(uint  k = 0; k < dimension; k++) {
    uint  ip = simplex_vert[is*dimension+k];
    uint  isov = point_list[ip];
