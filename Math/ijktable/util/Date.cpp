...
const char *MMyy              = "MM.yy";
const char *MMyyyy            = "MM.yyyy";
const char *ddMM              = "dd.MM";
const char *ddMMyy            = "dd.MM.yy";
const char *ddMMyyyy          = "dd.MM.yyyy";
const char *ss                = "ss";
const char *ssSSS             = "ss:SSS";
const char *mmss              = "mm:ss";
const char *mmssSSS           = "mm:ss:SSS";
const char *hhmm              = "hh:mm";
const char *hhmmss            = "hh:mm:ss";
const char *hhmmssSSS         = "hh:mm:ss:SSS";
const char *ddMMhhmm          = "dd.MM hh:mm";
const char *ddMMhhmmss        = "dd.MM hh:mm:ss";
const char *ddMMhhmmssSSS     = "dd.MM hh:mm:ss:SSS";
const char *ddMMyyhhmm        = "dd.MM.yy hh:mm";
const char *ddMMyyhhmmss      = "dd.MM.yy hh:mm:ss";
const char *ddMMyyhhmmssSSS   = "dd.MM.yy hh:mm:ss:SSS";
const char *ddMMyyyyhhmm      = "dd.MM.yyyy hh:mm";
const char *ddMMyyyyhhmmss    = "dd.MM.yyyy hh:mm:ss";
const char *ddMMyyyyhhmmssSSS = "dd.MM.yyyy hh:mm:ss:SSS";
...
Date::Date(int day, int month, int year) {
  if(!dateValidate(day, month, year)) {
    throwException("Invalid Date:%02d.%02d.%d", day, month, year);
  }
...
void Date::init(const char *src) {
  static const char *errmsg = "Invalid date-format <%s>";

  int i = 0;
  int d[3];
  for(Tokenizer tok(src, "./-"); tok.hasNext() && i < 3; i++) {
    if(sscanf(tok.next().c_str(), "%d", &d[i]) != 1) {
...
static void throwDateUnderflow() {
  throwException("Date becomes to small. First date in Gregorian calendar is %s", Date::getMinDate().toString().c_str());
}

static void throwDateOverflow() {
  throwException("Date becomes to big. Last date is %s", Date::getMaxDate().toString().c_str());
}
...
static void throwInvalidTimeComponent(const char *function, TimeComponent c) {
  throwException("%s:Invalid timeComponent (=%d).", function, c);
}
...
Date &Date::add(TimeComponent c, int count) {
  ...
  default   :
    throwInvalidTimeComponent(__FUNCTION__, c);
  }
  return *this;
}
...
int Date::get(TimeComponent c) {
  ...
  default         : throwInvalidTimeComponent(__FUNCTION__, c);
                    return 0;
  }
}
...
int Date::getWeeksInYear(int year) { // static
  ...
  default:
    throwException("%s(%d):Illegal weekDay (=%d) for %s", __FUNCTION__, year, lastDay.getWeekDay(), lastDay.toString().c_str());
    return 30;
  }
}
...
int Date::getDaysInMonth(int year, int month) { // static
  ...
  default:
    throwException("%s:Illegal month:%d. Must be [1..12].", __FUNCTION__, month);
    return 30;
  }
}
...
Date Date::getEaster(int year) { // static
  if(year < 326 || year > 9999) {
    throwException("Easter not defined for year=%d. Year must be in the range [326..9999]", year);
  }
...
#define CASECH(ch, comp) case ch: {                         \
  ...
  string tmp = ::format("%0*d", count, (comp) % scale); \
  strcpy(t, tmp.c_str());                                   \
  t += tmp.length();                                        \
}                                                           \
break

char *Date::tostr(char *dst, const char *format) const {
  int year, month, day;
  getDMY(day, month, year);
  char *t  = dst;
  for(const char *cp = format; *cp;) {
    switch(*cp) {
    CASECH('y', year          );
    CASECH('M', month         );
    CASECH('w', getWeek()     );
