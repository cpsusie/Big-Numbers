...
void Timestamp::init(const TCHAR *src) {
  ...
  Date date(dateStr);
  Time time = tok.hasNext() ? Time(tok.next()) : Time("00:00:00");
...
static void throwTimestampUnderflow() {
  throwException("Timestamp becomes to small. First date in Gregorian calendar is %s", Date::getMinDate().toString().c_str());
}

static void throwTimestampOverflow() {
  throwException("Timestamp becomes to big. Last date is %s", Date::getMaxDate().toString().c_str());
}
