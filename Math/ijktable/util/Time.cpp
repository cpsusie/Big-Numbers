...
Time::Time(int hour, int minute, int second, int millisecond) {
  if(!timeValidate(hour, minute, second, millisecond)) {
    throwException("Time::Invalid time:%02d:%02d:%02d.%d", hour, minute, second, millisecond);
  }
  m_factor = getFactor(hour, minute, second, millisecond);
}

void Time::throwInvalidFactor(INT64 factor) { // static
  throwException("Time::Invalid factor:%I64d. Must be in the range [0..%d[", factor, getMaxFactor());
}

void Time::init(const TCHAR *src) {
  static const TCHAR *errmsg = "Invalid time-format <%s>";

  int i = 0;
  int d[4];
  memset(d, 0, sizeof(d));
  for(Tokenizer tok(src, ".:-"); tok.hasNext() && i < 4; i++) {
