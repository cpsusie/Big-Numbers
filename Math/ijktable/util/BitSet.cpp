...
string BitSet::indexOutOfRangeString(size_t index) const {
  return format("Index %s out of range. Capacity=%s", format1000(index).c_str(), format1000(m_capacity).c_str());
}

void BitSet::throwIndexOutOfRange(const char *method, size_t index, _In_z_ _Printf_format_string_ char const * const format, ...) const {
  va_list argptr;
  va_start(argptr, format);
  const string msg = vformat(format, argptr);
  va_end(argptr);
  throwInvalidArgumentException(method, "%s:%s", msg.c_str(), indexOutOfRangeString(index).c_str());
}
...
BitSet::BitSet(size_t capacity) {
  if(capacity == 0) {
    throwInvalidArgumentException(__FUNCTION__, "Capacity=0");
  }
  m_capacity = capacity;
...
BitSet &BitSet::remove(size_t a, size_t b) {
  if(b >= m_capacity) {
    throwIndexOutOfRange(__FUNCTION__, b, "(%s,%s)", format1000(a).c_str(), format1000(b).c_str());
  }
  if(a > b) {
    return *this;
  }
...
intptr_t BitSet::getIndex(size_t i) const {
  ...
  const uchar frac = i%_BS_BITSINATOM;
  if(frac) {
...
string BitSet::toString(AbstractStringifier<size_t> *sf, const char *delim) const {
  string result = "(";
