...
static short mi[MAXPOINTS], mo[MAXPOINTS];
...
#define compare(i,j) (site_num->getIndex(i)-site_num->getIndex(j))
...
#define snkey(x) site_num->getIndex((x)->vert)

class VisitAddToFg : public SimplexVisitor {
private:
  fg *m_faces_gr_t;
  fg *find_fg(simplex *s, int q);
public:
  VisitAddToFg() : m_faces_gr_t(NULL) {
  }
  void *visit(simplex *s, void *);
  inline fg *getFacesg() {
    return m_faces_gr_t;
  }
};

fg *VisitAddToFg::find_fg(simplex *s, int q) {
  fg *f;
...
void *VisitAddToFg::visit(simplex *s, void *dum) {
...
fg *build_fg(simplex *root) {
  VisitAddToFg addVisitor;
  visit_hull(root, addVisitor);
  return addVisitor.getFacesg();
}

void visit_fg_i(void(*v_fg)(Tree *, int, int), Tree *t, int depth, int vn, int boundary) {
  int boundaryc = boundary;
  if(!t) return;
  Assert(t->fgs);
  if(t->fgs->mark != vn) {
    t->fgs->mark = vn;
    if(t->key != infinity && !mo[site_num->getIndex(t->key)]) {
      boundaryc = 0;
    }
...
int visit_fg_i_far(void(*v_fg)(Tree *, int), Tree *t, int depth, int vn) {
  int nb = 0;
  ...
  if(t->fgs->mark != vn) {
    t->fgs->mark = vn;
    nb = (t->key == infinity) || mo[site_num->getIndex(t->key)];
...
void p_fg(Tree* t, int depth, int bad) {
  ...
  fa[depth] = site_num->getIndex(t->key);
  for(i = 0; i <= depth; i++) {
...
class VisitMarkPoints : public SimplexVisitor {
public:
  void *visit(simplex *, void *);
};

void *VisitMarkPoints::visit(simplex *s, void *dum) {
  int i, snum;
  neighbor *sn;
  ...
  for(i = 0, sn = s->neigh; i < cdim; i++, sn++) {
    ...
    snum = site_num->getIndex(sn->vert);
    if(s->mark) {
      mo[snum] = 1;
    } else {
      mi[snum] = 1;
...
bool static check_ashape(simplex *root, double alpha) {
  for(int i = 0; i < MAXPOINTS; i++) {
    mi[i] = mo[i] = 0;
  }
  VisitSetMark m0(0), m1(1);
  visit_hull(root, m0);
  alphaTest.test(0, 0, &alpha);
  visit_outside_ashape(root, m1);
  VisitMarkPoints mp;
  visit_hull(root, mp);
...
double find_alpha(simplex *root) {
 double al=0,ah=0,am;
  ...
  for(int i = 0; i < pdim; i++) {
    ah += (maxs[i] - mins[i])*(maxs[i] - mins[i]);
  }
...
void print_hist_fg(simplex *root, fg *faces_gr, FILE *F) {
  ...
  for(k = 0; k <= i; k++) {
    ...
    fprintf(F, "%d/%d/%d", (int)tot_far[k], (int)tot_good[k], (int)tot_good[k] + (int)tot_bad[k]);
  }
