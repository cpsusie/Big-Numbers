...
int main(int argc, char **argv) {
  ...
  try {
    ...
    if(isotable[0].NumTableEntries() != isotable[1].NumTableEntries()) {
      cerr << "Table size mismatch:" << endl;
      for(int i = 0; i < 2; i++) {
        cerr << "  " << fname[i] << " has " 
             << isotable[i].NumTableEntries() << " entries."
             << endl;
      }
      exit(20);
    }

    if(isotable[0].NumVerticesPerSimplex() != isotable[1].NumVerticesPerSimplex()) {
    ...
    for(int it = 0; it < isotable[0].NumTableEntries(); it++) {
      if(isotable[0].NumSimplices(it) != isotable[1].NumSimplices(it)) {
        ...
        for(int i = 0; i < 2; i++) {
          cerr << "  " << fname[i] << " has " 
               << isotable[i].NumSimplices(it) << " simplices."
               << endl;
        }
        continue;
      }
      ...
      for(int is0 = 0; is0 < isotable[0].NumSimplices(it); is0++) {
        ...
        if(!found_matching_simplex) {
          cerr << "Table entry " << it << " mismatch:"
               << endl;
          cerr << "  " << fname[0] << ", simplex " << is0
               << " has no match in " << fname[1] << "."
               << endl;
          continue;
        }
      }
...
void diff_table_polyhedra(const ISOSURFACE_TABLE isotable[2]) {
  ...
  assert(isotable[0].Polyhedron().Dimension() == isotable[1].Polyhedron().Dimension());

  if(isotable[0].Polyhedron().NumVertices() != isotable[1].Polyhedron().NumVertices()) {
  ...
  if(isotable[0].Polyhedron().NumEdges() != isotable[1].Polyhedron().NumEdges()) {
    cerr << "Polyhedron number of edges mismatch:" << endl;
    for(int i = 0; i < 2; i++) {
      cerr << "  " << fname[i] << " has " 
           << isotable[i].Polyhedron().NumEdges() << " edges."
           << endl;
    }
    exit(20);
  }
  ...
  // do not halt if facet differences are found
  if(isotable[0].Polyhedron().NumFacets() != isotable[1].Polyhedron().NumFacets()) {
    cerr << "Polyhedron number of facets mismatch:" << endl;
    for(int i = 0; i < 2; i++) {
      cerr << "  " << fname[i] << " has " 
           << isotable[i].Polyhedron().NumFacets() << " facets."
           << endl;
    }
    return;
  }
  ...
  for(int jf = 0; jf < isotable[0].Polyhedron().NumFacets(); jf++) {
    if(isotable[0].Polyhedron().NumFacetVertices(jf) != isotable[1].Polyhedron().NumFacetVertices(jf)) {
    ...
    if(isotable[0].Polyhedron().Facet(jf) != isotable[1].Polyhedron().Facet(jf)) {
      cerr << "Polyhedron facet vertices mismatch:" << endl;
      for(int iv = 0; iv < isotable[0].Polyhedron().NumVertices(); iv++) {
        if(isotable[0].Polyhedron().IsVertexInFacet(jf, iv) != isotable[1].Polyhedron().IsVertexInFacet(jf, iv)) {
          for(int i = 0; i < 2; i++) {
            if(isotable[i].Polyhedron().IsVertexInFacet(jf, iv)) {
              cerr << "  " << fname[i] << ", facet " << jf
                   << " has vertex " << iv << "."
                   << endl;
            } else {
              cerr << "  " << fname[i] << ", facet " << jf
                   << " does not have vertex " << iv << "."
                   << endl;
            }
          }
        }
...
void sort_simplex_vertices(const ISOSURFACE_TABLE & isotable, const int it, const int is, int * vlist) {
