...
ISOSURFACE_TABLE_POLYHEDRON::ISOSURFACE_TABLE_POLYHEDRON(const ISOSURFACE_TABLE_POLYHEDRON & init) {
  Init();
  *this = init;
}

ISOSURFACE_TABLE_POLYHEDRON & ISOSURFACE_TABLE_POLYHEDRON::operator=(const ISOSURFACE_TABLE_POLYHEDRON & right) {
  if(&right != this) {
    FreeAll();
...
void ISOSURFACE_TABLE_POLYHEDRON::SetNumEdges(const int nume) {
  ...
  if(NumVertices() == 0)
    throw PROCEDURE_ERROR(procname, "Number of vertices must be set before number of edges.");
...
void ISOSURFACE_TABLE_POLYHEDRON::SetNumFacets(const int numf) {
  ...
  if(NumVertices() == 0)
    throw PROCEDURE_ERROR(procname, "Number of vertices must be set before number of facets.");
...
void ISOSURFACE_TABLE_POLYHEDRON::SetNumFacetVertices(const FACET_INDEX jf, const int numv) {
  const char * procname = "ISOSURFACE_TABLE_POLYHEDRON::SetNumFacetVertices";
...
void ISOSURFACE_TABLE_POLYHEDRON::SetVertexCoord(const int iv, const int ic, const int coord) {
  ...
  if(coord%2 != 0)
    throw PROCEDURE_ERROR(procname, "Illegal vertex coordinate.  Vertex coordinate must be even.");

  if(vertex_coord == NULL)
    throw PROCEDURE_ERROR(procname, "Vertex coordinate memory not allocated.");
...
void ISOSURFACE_TABLE_POLYHEDRON::SetEdge(const EDGE_INDEX ie, const int iv0, const int iv1) {
  ...
  if(iv0 < 0 || iv0 > NumVertices() || iv1 < 0 || iv1 > NumVertices())
    throw PROCEDURE_ERROR(procname, "Illegal vertex index.");
...
void ISOSURFACE_TABLE_POLYHEDRON::SetFacetVertex(const FACET_INDEX jf, const int k, const int iv) {
  ...
  if(k < 0 || k >= NumFacetVertices(jf)) 
    throw PROCEDURE_ERROR(procname, "Illegal index to list of facet vertices.");
...
//   is an integer
int ISOSURFACE_TABLE_POLYHEDRON::MidpointCoord(const EDGE_INDEX ie, const int ic) const {
...
void ISOSURFACE_TABLE_POLYHEDRON::GenCube(const int cube_dimension) {
  ...
  for(int ic = 0; ic < Dimension(); ic++) {
    ...
    if(k0 != NumFacetVertices(jf0) || k1 != NumFacetVertices(jf1))
      throw PROCEDURE_ERROR(procname, "Programming error in facet generation.");
...
bool ISOSURFACE_TABLE_POLYHEDRON::Check(ERROR & error_msg) const {
  if(!CheckDimension()) {
    error_msg.ClearAll();
    error_msg.AddMessage("Illegal polyhedron dimension ", Dimension(), ".");
    return(false);
  }
  ...
  for(int ie = 0; ie < NumEdges(); ie++) {
    for(int ip = 0; ip < 2; ip++) {
      ...
      if(iv < 0 || iv >= NumVertices()) {
        error_msg.ClearAll();
        error_msg.AddMessage("Illegal edge endpoint ", iv, " for edge ", ie, ".");
        return(false);
      }
    }
  }
...
bool ISOSURFACE_TABLE::ISOSURFACE_TABLE_ENTRY::Check(ERROR & error_msg) const {
  if(num_simplices < 0) {
    error_msg.ClearAll();
    error_msg.AddMessage("Number of simplices in isosurface table entry must be non-negative.");
    return(false);
  }
...
// constructor
// d = dimension of space containing isosurface.  Should be 2, 3 or 4.
ISOSURFACE_TABLE::ISOSURFACE_TABLE(const int dimension, const int simplex_dimension):polyhedron(dimension) {
...
void ISOSURFACE_TABLE::CheckIsoVerticesAlloc(const char * procname, const int vstart, const int numv) {
  if(numv == 0) return;

  if(isosurface_vertex == NULL) {
    throw PROCEDURE_ERROR(procname, "Set number of isosurface vertices before storing vertices.");
  }

  if(numv+vstart > NumIsosurfaceVertices()) {
    throw PROCEDURE_ERROR(procname, "Illegal isosurface vertex index.");
  }
}
...
void ISOSURFACE_TABLE::SetSimplexVertex(const TABLE_INDEX it, const int is, const int k, const ISOSURFACE_VERTEX_INDEX isov) {
...
bool ISOSURFACE_TABLE::CheckTable(ERROR & error_msg) const {
  ...
  for(int it = 0; it < NumTableEntries(); it++)
    if(!entry[it].Check(error_msg)) {
      error_msg.AddMessage("Error detected at isosurface table entry ", it, ".");
      return(false);
    }
    ...
    for(int is = 0; is < NumSimplices(jt); is++)
      for(int iv = 0; iv < NumVerticesPerSimplex(); iv++) {
        ...
        if(iso_v < 0 || iso_v >= NumIsosurfaceVertices()) {
          error_msg.ClearAll();
          error_msg.AddMessage("Illegal isosurface vertex ", iso_v, " in isosurface table entry ", jt, "."); 
          return(false);
        }
      }
...
bool ISOSURFACE_EDGE_TABLE::ISOSURFACE_EDGE_TABLE_ENTRY::Check(ERROR & error_msg) const {
  if(num_edges < 0) {
    error_msg.ClearAll();
    error_msg.AddMessage("Number of edges in isosurface table entry must be non-negative.");
    return(false);
  }
  ...
  for(int jt = 0; jt < NumTableEntries(); jt++)
    for(int ie = 0; ie < NumEdges(jt); ie++)
      for(int iend = 0; iend < 2; iend++) {
        ...
        if(iso_v < 0 || iso_v >= NumIsosurfaceVertices()) {
          error_msg.ClearAll();
          error_msg.AddMessage("Illegal isosurface vertex ", iso_v, " in isosurface table edge entry ", jt, ".");
          return(false);
        }
      }
...
void ISOSURFACE_EDGE_TABLE::GenEdgeLists() {
  ...
  for(int it = 0; it < NumTableEntries(); it++) {
    ...
    for(int is = 0; is < NumSimplices(it); is++) {
      ...
      sort((EDGE_INDEX*)vlist, vlist+Dimension());
...
void ISOSURFACE_TABLE_AMBIG_INFO::Alloc(const long num_table_entries) {
...
void ISOSURFACE_TABLE_AMBIG_INFO::ComputeAmbiguityInformation(const ISOSURFACE_TABLE & isotable) {
  ...
  IJK::PROCEDURE_ERROR error(__FUNCTION__);
...
void ISOSURFACE_TABLE_AMBIG::ComputeAmbiguousFacets(const int * vertex_sign, FACET_SET & facet_set, FACET_INDEX & num_ambiguous_facets) const {
...
ulong  IJKTABLE::calculate_num_entries(const int num_vert, const int num_colors) {
  ...
  for(int iv = 0; iv < num_vert; iv++) {
    if(num_table_entries > max2) {
      throw PROCEDURE_ERROR(__FUNCTION__, "Number of entries is too large.");
    }
...
bool IJKTABLE::is_poly_ambiguous(const IJKTABLE::ISOSURFACE_TABLE_POLYHEDRON & poly, const int * vertex_sign) {
...
bool IJKTABLE::is_facet_ambiguous(const IJKTABLE::ISOSURFACE_TABLE_POLYHEDRON & poly, const IJKTABLE::FACET_INDEX ifacet, const int * vertex_sign) {
  const int numv = poly.NumVertices();
  int       num0 = 0;
  int       num1 = 0;
...
/// @param vertex_sign[i] = Sign of isosurface vertex i.
int IJKTABLE::compute_num_connected(const IJKTABLE::ISOSURFACE_TABLE_POLYHEDRON & poly, const int iv, const int * vertex_sign) {
  const int numv  = poly.NumVertices();
...
/// @param vertex_sign[i] = Sign of isosurface vertex i.
void IJKTABLE::compute_ambiguous_facets(const ISOSURFACE_TABLE_POLYHEDRON & poly, const int * vertex_sign, FACET_SET & facet_set, FACET_INDEX & num_ambiguous_facets) {
...
/// @param vertex_sign[i] = Sign of isosurface vertex i.
int IJKTABLE::compute_num_connected_in_facet(const IJKTABLE::ISOSURFACE_TABLE_POLYHEDRON & poly, const FACET_INDEX jf, const int iv, const int * vertex_sign) {
  const int numv = poly.NumVertices();
  const int nume = poly.NumEdges();
  const int isign = vertex_sign[iv];
  fixedarray<bool> visited(numv);
